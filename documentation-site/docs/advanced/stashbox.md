...
| A community-curated metadata source | Owned by Obscura |
| Where fingerprint identification happens | A scraper that takes URLs |

StashBox endpoints live in the `stashbox_endpoints` table and are queried by the client at `packages/stash-import/src/stashbox/client.ts`. They are not in `plugin_packages`. They appear in the same Identify provider pickers as native plugins because the cascade UI is provider-agnostic.
...
Click **Test** — Obscura issues a probe query to verify the URL + key. If it returns OK, the endpoint is healthy. Hit **Save**.
...
1. The engine collects every fingerprint Obscura has for the video — `md5`, `oshash`, `phash` (whichever are populated).
...
Accepting a StashBox-origin match links your local video to the remote scene (`stash_ids` row) **and** queues a fingerprint submission. Each algorithm Obscura has for that video is submitted to the endpoint, helping the next person identify the same content.
...
API keys are stored encrypted in `stashbox_endpoints.api_key` using the same `OBSCURA_SECRET` env var that protects plugin auth. **Set `OBSCURA_SECRET` in production** so keys survive container recreations:
...
services:
  obscura:
    ...
    environment:
      OBSCURA_SECRET: <a-long-random-string>
```

If `OBSCURA_SECRET` changes between restarts, encrypted values become unreadable and you'll have to re-enter every API key.
