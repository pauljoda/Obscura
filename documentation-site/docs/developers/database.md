...
Obscura stores application state in PostgreSQL 16 and manages schema changes
through EF Core migrations in the .NET backend.

## Source of Truth

- Entity mappings live under `apps/backend/src/Obscura.Infrastructure/Persistence`.
- Migrations live under `apps/backend/src/Obscura.Infrastructure/Persistence/Migrations`.
- Startup applies pending migrations through the shared .NET runtime used by the
  API and worker.

Do not add Drizzle, `@obscura/db`, SvelteKit database code, or TypeScript
database migrations.
