...
Obscura is intentionally small: a .NET API, a .NET worker, a static Svelte
...
OpenAPI clients and small hand-written v2 wrappers where generation has not
caught up yet.
...
| `packages/stash-import` | Stash import adapters and normalizers. |
...
- Do not reintroduce Drizzle or `@obscura/db`.
- Do not reintroduce `@obscura/app-core` as a server package.
- Do not add a TypeScript worker.
- Server contracts belong in `apps/backend/src/Obscura.Contracts`.
