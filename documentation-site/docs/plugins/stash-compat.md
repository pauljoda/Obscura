...
description: Wrap a Stash YAML scraper for use as an Obscura plugin.
---

# Stash Compatibility

The Stash community has hundreds of YAML-defined site scrapers — battle-tested for the long tail of porn-site metadata. Obscura runs them through a compatibility adapter so you don't have to rewrite them in TypeScript or Python.
...
Stash YAML scraper definition
   ...
   stash-compat manifest.yml  ─┐
                               │
                               ▼
   packages/stash-import/src/stash-adapter.ts
                               │
                               ▼
...
- Maps Obscura action names to Stash action names (`videoByURL` → `sceneByURL`, `videoByName` → `sceneByName`, `videoByFragment` → `sceneByFragment`, etc.).
- Runs the YAML scraper's HTTP / regex / xpath pipeline.
- Normalizes the output to Obscura's shape.
...
You declare which Obscura capabilities the wrapped scraper provides — capabilities the YAML doesn't actually implement will fail at execution time, so be honest.
...
Action mapping (Obscura → Stash):

| Obscura action | Stash action |
| --- | --- |
...
Other Obscura actions (`folderCascade`, `seriesCascade`, `audioByURL`, etc.) are not part of the Stash protocol; declaring them on a stash-compat plugin will fail at runtime.

## Auth and headers

Stash YAML supports HTTP headers and per-action driver options. The adapter wires these through transparently. If the scraper needs an API key or cookie, declare it as `auth` in the Obscura manifest:
...
- **Per-result tag deduplication policies** specific to Stash — the Obscura normalizer applies its own (case-insensitive for tags).
- **Stash UI integrations** (custom drawer behavior, in-Stash CSS) — Obscura uses its own cascade drawer.

If your YAML scraper relies on any of these, it'll need a port to native TypeScript or Python.

## Result mapping

Stash result fields are normalized to Obscura's shapes:

| Stash field | Obscura field (NormalizedVideoResult) |
| --- | --- |
| `Title` | `title` |
| `Date` | `date` |
| `Details` | `details` |
...
When a stash-compat plugin returns nothing or a bad result, the adapter writes the raw HTTP response body to the worker logs. `docker compose logs obscura | grep stash-adapter` is a good first look. Most failures are:
...
Most first-party Obscura plugins (TMDB, TVDB, MusicBrainz, YouTube) are native because they call structured APIs and use cascade flows. Most long-tail provider integrations remain stash-compat because the YAML community has already done the work.
