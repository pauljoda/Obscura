...
The values the user sets are stored encrypted in `plugin_auth.encrypted_value` and decrypted in-memory only at the moment of plugin execution. The `OBSCURA_SECRET` env var is the encryption key — set it in production so credentials survive container recreations.
