...
Each queue has a per-queue concurrency setting. By default, every queue runs one job at a time — Obscura prefers steady throughput over maximizing parallel CPU on a single user's box.
