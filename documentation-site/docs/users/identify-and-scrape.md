...
| --- | --- | --- |
| **Plugin** | A native Obscura provider with a manifest, capabilities, and an execution envelope. Written in TypeScript or Python. | `tmdb`, `tvdb`, `youtube`, `musicbrainz` |
| **Stash-compat scraper** | A YAML-defined Stash community scraper, wrapped by Obscura's adapter. | Most of the long tail of porn-site scrapers |
...
- **Obscura Index** — first-party plugin registry (TMDB, TVDB, YouTube, MusicBrainz). One-click install.
...
1. Open the **Obscura Index** tab.
2. Find the plugin you want.
3. Click **Install**. Obscura downloads the package, verifies its checksum, and writes it to the on-disk plugin directory. The new plugin appears under **Installed**.
