...
export const DEFAULT_DATABASE_URL = "postgresql://obscura:obscura@localhost:5432/obscura";
export const DEFAULT_BACKUP_DIR = ".obscura-dev/backups";
...
export function databaseParts(databaseUrl) {
  ...
  return {
    database: url.pathname.replace(/^\//, "") || "obscura",
    password: decodeURIComponent(url.password || "obscura"),
    port: url.port || "5432",
    user: decodeURIComponent(url.username || "obscura"),
  };
}
...
export function envForLocalPostgres(databaseUrl) {
  ...
  return {
    ...process.env,
    PGDATABASE: url.pathname.replace(/^\//, "") || "obscura",
    PGHOST: url.hostname || "localhost",
    PGPASSWORD: decodeURIComponent(url.password || ""),
    PGPORT: url.port || "5432",
    PGUSER: decodeURIComponent(url.username || "obscura"),
  };
}
