#!/usr/bin/env bash
#
# Kill orphaned Obscura dev processes (dotnet, node/vite, docs).
...
kill_matching ".NET API"        "Obscura\.Api"
kill_matching ".NET Worker"     "Obscura\.Worker"
kill_matching "dotnet watch"    "dotnet watch.*Obscura"

# Node / frontend
kill_matching "Vite (SvelteKit)" "vite.*web-svelte"
kill_matching "SvelteKit node"   "node.*web-svelte"
kill_matching "turbo"            "turbo.*dev.*obscura"
