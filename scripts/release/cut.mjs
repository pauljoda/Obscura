...
function findPackageJsons() {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      const pkgPath = join(rootAbs, entry, "package.json");
      try {
        if (statSync(pkgPath).isFile()) targets.push(pkgPath);
      } catch {}
    }
  }
  return targets;
...
if (phase === "release") {
  ...
  updatePackageVersions(releaseVersion);
