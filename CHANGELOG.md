...
- **`/design-language`** page (not linked in nav, `noindex`) — single-page gallery of Dark Room tokens: typography, buttons, badges, surfaces, accent/status palettes, LEDs, meters, loading patterns, motion, form controls, caption styles, and gradient placeholders. Includes a live preview of **`AppRouteLoading`** (the `(app)/loading.tsx` spinning square).
- **`AppRouteLoading`** component — shared implementation for the app shell route loading state so the design-language page can render the exact system loader.
