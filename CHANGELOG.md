...
- Refactored v2 .NET capability kinds into discovered concrete implementations with a dedicated registry, matching the entity-kind and relationship patterns instead of a static grouping class.
- Consolidated shared v2 .NET capability-kind metadata into an abstract base class so each concrete kind only declares its own code and display name.
