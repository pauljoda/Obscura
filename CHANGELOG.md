...
- **More missing artwork now falls back to Obscura gradients.** Studios without icons and sub-galleries without cover images now render through the same gradient fallback treatment as the rest of the library instead of showing broken or flat empty thumbnail panels.
...
- **Tags now use the same thumbnail component everywhere they appear.** A new `TagThumbnail` replaces the inline tag card snippet on `/tags` and the inline tag hero on `/tags/[id]`. Search results and the command palette now show full tag cards (gradient + cover image) instead of a plain icon row. Galleries and images on a tag detail page render through their own shared thumbnail components too.
- **Collections now use the same thumbnail components everywhere.** A new `CollectionThumbnail` covers the collection cards on `/collections` and the hero art on a collection detail page, so custom cover images and the fallback gradient look the same in both places. Inside a collection, each item now delegates to its entity-specific thumbnail (`VideoThumbnail`, `GalleryThumbnail`, `ImageThumbnail`, `AudioTrackThumbnail`) — video items on a collection gain hover trickplay, galleries gain preview-image scrub, images animate on hover when they have a preview, and audio tracks fall back to their library's cover. Collection items finally render with the same treatment they get on their own library pages.
- **Audio libraries and tracks now use the same thumbnail component everywhere they appear.** New `AudioLibraryThumbnail` and `AudioTrackThumbnail` components replace the hand-rolled album art on `/audio`, the homepage Recent Audio row, the Sub-Libraries grid on `/audio/[id]`, the audio section of a studio detail page, global search results, and the command palette. All of those places now get the same rotating-disc fallback, hover play button, NSFW chip, and track-count chip in a single implementation.
...
- **Collection detail pages now lead with their cover art.** Opening a collection shows the cover in a full hero treatment with a blurred backdrop, matching the audio library detail layout while keeping playlist and management actions up top.
...
- **Videos and Series preferences are now stored in Obscura's database instead of browser cookies + localStorage.** Sort, filters, search, view mode, and saved filter presets all move from the `obscura-videos-list` / `obscura-series-list` cookies and `obscura-videos-filter-presets` / `obscura-series-filter-presets` localStorage keys to the same `ui_prefs` table that Galleries/Images/Actors already use. Upgrade effect: any saved sort + filters + presets on `/videos` and `/series` reset once on upgrade; re-saving them writes to the new location and they then follow you across devices and browsers.
...
- **Galleries and images now use the same unified thumbnail component as videos and series.** A new `GalleryThumbnail` and `ImageThumbnail` pair handle covers, hover cycling through a gallery's interior images, and on-hover video preview playback for animated/video-format images (GIF, APNG, MP4, WebM, etc.). The same components are now used on the galleries and images libraries, gallery interior tiles, the homepage's Recent Galleries / Recent Images rows, global search results, and the command palette — so thumbnails look and feel the same everywhere they appear. You can also upload a custom cover image for any gallery (independent of picking one of its interior images), and a custom per-image thumbnail override; the asset server prefers these custom images over the auto-generated ones.
...
- **Library toggles now propagate through the visible app state.** Disabling a watched library, turning off one of its media types, or reclassifying a root as NSFW now updates the current Svelte tab immediately and removes or restores the affected library-backed content across videos, series, search, collections, and related counters without waiting for a hard refresh.

- **Star ratings are now click-to-rate everywhere, no Edit mode required.** A new `InlineRating` component does an optimistic update, calls the entity's update API, and reverts if the request fails. Audio libraries, audio tracks, gallery detail, image detail, performer detail, and the video detail header all share this behavior — and the rating field has been removed from the corresponding edit forms (video, audio library, audio track) since you don't need to enter edit mode just to set a rating.
- **The standardized form kit now powers every Svelte entity edit / create page.** `studios/new`, `performers/new`, `tags/new`, the video edit panel (used by both the inline metadata tab and the dedicated `videos/[id]/edit` route), and the audio library / audio track edit forms all run on the same `EditFormShell`, `TextField`, `TextAreaField`, `DateField`, `SearchSelect`, `TagSelect`, and `ToggleChip` components. Selectors for studio, performers, and tags get the same custom popover with search and "Add 'X'" inline-add behavior; flag toggles look the same; save/cancel buttons live in the same place; and the form kit also gained a `number` input variant for fields like episode and orgasm/like counts.
- **The Svelte audio edit forms are now built from a shared form kit.** A new `apps/web-svelte/src/lib/components/forms/` module provides `TextField`, `TextAreaField`, `DateField`, `SearchSelect` (single-value picker with search + "Add 'X'" inline-add), `TagSelect` (multi-chip picker with search + inline-add), `ToggleChip`, `FormActions`, and an `EditFormShell` wrapper. Audio library and audio track edit modes have been rebuilt on top of the kit so labels, save/cancel buttons, input styling, and the studio / artist / tag pickers all look and behave the same — and every future entity edit page can lean on the same components.
- **Svelte audio track rows are no longer absurdly tall.** The arbitrary `grid-cols-[…]` classes on the new playlist used commas as column separators, which Tailwind couldn't parse — the whole grid collapsed to a single column so every cell rendered on its own line and each row stretched past 170 px. Track rows now use underscore-separated column definitions, pack into proper 5-column grids, and sit at ~42 px each.
- **The Svelte audio library and track detail pages now share the same hero treatment.** Both pages lead with a full-width hero that uses the library cover as a blurred backdrop, a large cover square, an oversized title, and an inline action strip (Play All / Shuffle on the library, Add to collection / Edit on the track). The library's sidebar-style "Library Info" panel is gone — tracks, duration, studio, date, and organized status now sit inline in the hero meta row, and the folder path drops to a single monospace line below.
- **The Svelte audio track detail player now docks at the bottom like the library page.** The inline player that sat under the hero was removed; playback now uses the same floating bottom transport as `/audio/[id]` so controls stay available as you scroll through the track's details and metadata.
- **The Svelte audio pages got a visual pass.** The audio library list now shows hover-reveal play buttons with a brass glow, a scrolling disc animation on missing covers, and a track-count badge over the art. Library detail swaps the old track table for a modern playlist where the active track gets an accent bar, a live equalizer, and a subtle gradient highlight. Audio track detail has been completely overhauled into a full-width hero with blurred cover backdrop, large title / artist / album, inline rating, and a two-column details grid with a dedicated Technical sidebar.
- **The floating Svelte audio player no longer jumps in front of the shell or start on its own.** Opening an audio library now leaves playback idle until you choose a track, and the bottom transport sits underneath the sidebar layers instead of covering slide-out navigation.
...
- **SFW mode no longer leaks NSFW titles through the Jobs page.** Active, failed, completed, and recent job cards on `/jobs` now hide any job whose target is an NSFW-marked video, series, audio track, or library root when the UI is in SFW mode. Titles, trigger descriptions, and error output for those jobs were previously visible even with NSFW set to Off. Technical probe / fingerprint / preview jobs on SFW entities still show normally.
...
- **Actor Known For now groups repeated Series roles.** When an actor has the same role across multiple episodes of one Series, the Known For row now shows one Series card that opens the Series instead of repeating every episode.
...
- **Identify no longer force-flags content NSFW.** Previously, accepting a video identify result always set the video, its newly-created studio, any new performers, and any new tags to NSFW — even when the match came from a general-purpose Obscura plugin like YouTube or TMDB. Accept now reads the source provider: matches from StashBox endpoints or stash-compat community scrapers stay NSFW by default (those are porn-metadata protocols by design), but Obscura plugins defer to the plugin manifest's `isNsfw` flag, so YouTube / TMDB matches no longer silently move clean content into your NSFW-filtered view.
- **Identify "Accept" buttons now apply video results from plugins.** Previously, when a plugin (e.g. TMDB, YouTube) produced a video match on the Identify page's Videos tab, clicking **Accept** did nothing — the plugin had saved the scrape result under a generic `video` entity type that neither the `/scrapers/results/:id/accept` nor the `/plugins/results/:id/accept` endpoints would apply. Plugin saves now detect whether the target is an episode or a movie and stamp the specific `video_episode` / `video_movie` type so the accept path lands the patch on the right table. Accept failures across every Identify tab also surface an error state on the row instead of silently reverting, so you can see *why* an accept didn't stick.
...
- `BulkActionBar.svelte` — shared list-mode bulk action row used by the main library pages for visible-item selection, Mark NSFW, Delete, and Clear actions.
...
- `CollectionThumbnail.svelte` — shared collection cover renderer (custom image + gradient fallback with `FolderOpen` icon). Used on `/collections` cards and the hero of `/collections/[id]`. `CollectionItemCard` was refactored to delegate to `VideoThumbnail` / `GalleryThumbnail` / `ImageThumbnail` / `AudioTrackThumbnail` for its items instead of rendering its own `<img>` tag.
...
- Resolution and file size sort columns on `/api/images`. Images can now be sorted by resolution (`sort=resolution`, proxied through the width column) or file size (`sort=size`).
- `GalleryEdit.svelte` and `ImageEdit.svelte` — standardized edit components built on the shared `EditFormShell` / `TextField` / `SearchSelect` / `TagSelect` / `ToggleChip` / `DateField` form kit already used by videos and audio. `GalleryEdit` includes a three-option cover-art editor (upload, pick from gallery images, clear); `ImageEdit` includes a thumbnail uploader with revert. The `/galleries/[id]` and `/images/[id]` pages now expose an **Edit** button that reveals these forms inline.
- `GalleryThumbnail` and `ImageThumbnail` shared Svelte components. Both mirror the feature-parity baseline of `VideoThumbnail` / `SeriesThumbnail`: size variants (grid / list / compact / hero), NSFW blur + chip, fallback icons, cache-busting via `updatedAt`. `ImageThumbnail` plays the image's `preview.mp4` on hover for video / animated image formats; `GalleryThumbnail` cycles through `previewImagePaths` on hover (mouse-X mapped to interior-image index) and has an optional stacked-preview card variant for library grids.
- `POST/DELETE /api/images/[id]/thumbnail` — upload or reset a custom per-image thumbnail override. Stored as `{cache}/images/{id}/thumb-custom.jpg`; the asset resolver now prefers this over the auto-generated `thumb.jpg` when present.
- `POST /api/galleries/[id]/cover/upload` — upload a custom gallery cover file directly (alternative to the existing image-selection `POST /api/galleries/[id]/cover`). Stored as `{cache}/galleries/{id}/cover-custom.jpg`; the asset resolver prefers the custom file over the linked `coverImageId`'s thumbnail. `DELETE /api/galleries/[id]/cover` now also removes any custom upload.
- `setCustomImageThumbnailWrite`, `resetImageThumbnailWrite`, `uploadGalleryCoverWrite` in `@obscura/app-core`, plus `getGeneratedGalleryDir` in `@obscura/media-core`, to back the new endpoints.
- `uploadImageThumbnail`, `resetImageThumbnail`, `uploadGalleryCover`, `setGalleryCoverFromImage`, `deleteGalleryCover` client helpers in `$lib/api/media` so UI code can call the new routes without reaching for `fetch` directly.

- Trickplay hover scrub on video thumbnails across the Svelte app, ported from the legacy web app. A new `VideoThumbnail` component and shared `createTrickplayScrub` state (backed by the existing `@obscura/ui-svelte` trickplay helpers) power every `VideoCard` variant (grid / list / compact). The scrub timestamp now floats in a chip above the progress bar, clamped inside the card bounds, instead of the fixed top-left badge used on the old web app.
- `VideoCardListItemDto` now carries `spritePath` and `trickplayVttPath` so the lean card payload used by the home dashboard, `/videos`, and search can drive hover scrub without falling back to the richer `VideoListItemDto`.
- Home dashboard's "Recent Videos" row, the `/search` results list, and the command palette now render video entries through `VideoCard` (grid + compact) instead of ad-hoc tiles, so they inherit trickplay scrub, NSFW blur, CC/resolution/codec chips, and episode badges automatically.
...
- The SvelteKit app now owns every video-marker route: `GET/POST /api/videos/:id/markers`, `PATCH/DELETE /api/videos/markers/:markerId`. Shared `listMarkersRead` / `createMarkerWrite` / `updateMarkerWrite` / `deleteMarkerWrite` live in `@obscura/app-core` and both hosts delegate. A new `@obscura/app-core/errors` module introduces generic `NotFoundError` / `ValidationError` / `UpstreamError` / `ConflictError` sentinels so new entities can reuse one set of error classes and Svelte gets a single `mapAppCoreErrorToJson` helper for 400 / 404 / 409 / 502 mappings.
- The SvelteKit app is now the single ingress for the API during the remainder of the migration. `PUBLIC_API_URL` now defaults to `/api`, so browser `fetchApi(...)` calls land on SvelteKit itself; a new catch-all `apps/web-svelte/src/routes/api/[...rest]/+server.ts` transparently forwards any path SvelteKit doesn't own locally to the Fastify instance (via `INTERNAL_API_URL`). Locally-owned routes still take precedence over the catch-all, so migrated slices keep their byte-identical Svelte-served payloads. Once every Fastify route has a Svelte counterpart, the proxy and Fastify app can be deleted.
- The SvelteKit app now owns the full performer write surface: `POST /api/performers`, `PATCH/DELETE /api/performers/:id`, `PATCH /:id/favorite`, `PATCH /:id/rating`, `POST/DELETE /:id/image`, `POST /:id/image/from-url`. Shared `createPerformerWrite` / `updatePerformerWrite` / `deletePerformerWrite` / `setPerformerFavoriteWrite` / `setPerformerRatingWrite` / `uploadPerformerImageWrite` / `setPerformerImageFromUrlWrite` / `deletePerformerImageWrite` in `@obscura/app-core` own the transaction-based tag sync on create/update; `PerformerNotFoundError` / `PerformerValidationError` / `PerformerUpstreamError` sentinels let each host map to its own error response. Parity-verified on POST + PATCH + favorite + 404.
- The SvelteKit app now owns the full studio write surface: `POST /api/studios`, `PATCH/DELETE /api/studios/:id`, `PATCH /:id/favorite`, `PATCH /:id/rating`, `POST/DELETE /:id/image`, `POST /:id/image/from-url`. Nine writes live in `@obscura/app-core`, including `findOrCreateStudioWrite` with recursive parent resolution and cycle detection, plus `StudioNotFoundError` / `StudioValidationError` / `StudioUpstreamError` sentinels. Parity-verified on POST + PATCH + favorite + 404.
- The SvelteKit app now owns the full tag write surface: `POST /api/tags`, `PATCH/DELETE /api/tags/:id`, `PATCH /api/tags/:id/favorite`, `PATCH /api/tags/:id/rating`, `POST/DELETE /api/tags/:id/image`, and `POST /api/tags/:id/image/from-url`. All eight writes move into `@obscura/app-core` as sentinel-error-throwing helpers plus `TagNotFoundError` / `TagValidationError` / `TagUpstreamError` classes, and Fastify and SvelteKit each translate sentinels into their own HTTP responses. Parity-verified on CRUD + favorite + rating + validation + 404 paths.
- The SvelteKit app now owns `GET/POST /api/libraries`, `PATCH/DELETE /api/libraries/:id`, and `GET /api/libraries/browse` locally. All five library-root routes move through new shared helpers in `@obscura/app-core`: `listLibraryRootsRead`, `createLibraryRootWrite`, `updateLibraryRootWrite`, `deleteLibraryRootWrite`, plus `browseDirectories` / `verifyDirectory` / `labelForPath` / `syncMediaNsfwWithLibraryRoot`. Both Fastify and SvelteKit delegate, so behavior — directory validation, default scan flags, path resolution, media NSFW propagation on PATCH, and 404 error shape — stays in lockstep. Parity-verified on all five routes plus error paths (bad-path POST, ENOENT browse, unknown-id DELETE).
- The SvelteKit app now owns the `PUT /api/settings/library` mutation locally — the first write-side slice of the migration. The validation, normalization, and clamping logic (worker concurrency, subtitle style / font scale / position / opacity, playback mode) moves into `@obscura/app-core` as `updateLibrarySettingsWrite(db, payload)`, and both Fastify and SvelteKit delegate to it. Parity-verified: same payload → byte-identical updated row from both hosts, and out-of-range values clamp identically (e.g. `subtitleOpacity: 5` → `1`, `subtitleStyle: "invalid"` → `"stylized"`).
- The SvelteKit app now owns `GET /api/jobs` locally. The Operations dashboard read path — queue counts, latest scan timestamp, active/failed/completed/recent job lists with SFW filtering, and schedule settings — moves into `@obscura/app-core` as `getJobsDashboardRead(db, sfwOnly)`, and both Fastify and SvelteKit delegate to it. `fetchJobsDashboard` in the Svelte server helper now hits `/api/jobs` instead of Fastify. Svelte honors both `?nsfw=off` and the `x-obscura-nsfw-mode` header. Parity-verified across no-filter, query-param, and header-based SFW variants.
- The SvelteKit app now owns `GET /api/search` locally. The entire search provider registry (video-series, videos, performers, studios, tags, galleries, images) moves into `@obscura/app-core` with each provider expressed as a factory that accepts a Drizzle DB handle (`createSearchProviders(db)`), and both Fastify and SvelteKit bind the registry to their own DB runtime. `executeSearch` also moves into app-core so both hosts share scoring, ranking, and multi-kind/single-kind preview semantics. Parity-verified — plain query, SFW filter, single-kind, and multi-kind preview all return byte-identical payloads modulo the telemetry-only `durationMs` field.
- The SvelteKit app now also owns `GET /api/tags/:id`, `GET /api/studios/:id`, and `GET /api/performers/:id` locally. Detail reads (including the studio parent/child aggregate and the performer `knownFor` union across series/movies/episodes) move into `@obscura/app-core` as `getTagByIdRead`, `getStudioByIdRead`, and `getPerformerByIdRead`; the Fastify services delegate to the same helpers. 404 error bodies on the Svelte side now match Fastify's `{ error }` shape so the two hosts stay interchangeable for clients. Parity-verified across SFW on/off and 404 cases.
- The SvelteKit app now owns `GET /api/tags`, `GET /api/studios`, and `GET /api/performers` locally. All three list endpoints route through shared `@obscura/app-core` helpers (`listTagsRead`, `listStudiosRead`, `listPerformersRead`) that accept a Drizzle DB handle explicitly, and the Fastify services delegate to the same helpers. The appearance-count correlated subqueries used by all three reads also moved into `@obscura/app-core`, so Fastify and Svelte stay in lockstep on SFW filtering, pagination, sorting, and rating/country/image/video-count filters. Parity-verified — all variants (SFW on/off, sort by name/videos/rating/recent, search, pagination) return byte-identical payloads between the two hosts.
- The SvelteKit app now owns `GET /api/plugins/packages` locally. Identify and Settings flows can read installed Obscura plugins — with per-plugin `authStatus` ("ok" / "missing" / `null`) derived from each manifest's required auth fields and the configured `plugin_auth` rows — through the new Svelte route instead of Fastify. The shared `mapInstalledPluginPackages` helper lives in `@obscura/app-core`, and Fastify now delegates to it so both apps return byte-identical payload shapes.
- The SvelteKit app now owns `GET /api/scrapers/packages` and `GET /api/stashbox-endpoints` locally. Settings and Identify flows can read installed community scrapers and configured StashBox endpoints through the new local routes instead of Fastify. The shared mappers (`mapInstalledScraperPackages`, `mapStashBoxEndpointList`) live in `@obscura/app-core`, and Fastify now delegates to them so both apps return byte-identical shapes — including the enforced `isNsfw: true` flag and the masked-key preview on every StashBox endpoint.
...
- SvelteKit `/jobs` page now passes the caller's NSFW mode on both the SSR `+page.server.ts` load (read from the `obscura-nsfw-mode` cookie) and the client-side 5s poll. NSFW-targeted jobs are no longer visible in the Live Work, Failures, or Recently Finished lists when SFW mode is on.
- `POST /jobs/clear-metadata` wipes every scrape-derived custom field on `video_episodes`, `video_movies`, `video_series`, and `video_seasons` (overview, tagline, air/release date, runtime, rating, studio, performers-by-join, tags-by-join, posters/backdrops/logos, URLs, external IDs, `organized=false`), deletes every join row in the `video_{episode,movie,series}_{performers,tags}` tables and the full `scrape_results` log, and queues a `library-scan` job on every enabled `library_roots` row. Technical probe data (duration, codecs, fingerprints, thumbnails / previews / sprites) is intentionally left in place. Returns `{ episodesCleared, moviesCleared, seriesCleared, librariesQueued, librariesSkipped }`. Surfaced in the SvelteKit `/settings` → Diagnostics section as "Clear all metadata" with a confirm dialog.
- `apps/web-svelte/` cascade review drawer polish: the `ImagePicker` modal now uses `bg-bg/95 backdrop-blur-md` with `surface-elevated` inside (was `bg-bg/80 backdrop-blur` + `glass-1`, which let the drawer behind bleed through), and every raw `<input type="checkbox">` inside the cascade editor (series / season / episode field-mask grids, season accept toggle, episode accept toggle, ImagePicker language-agnostic filter) has been swapped to the styled `<Checkbox>` primitive so checked state shows the brass-glow fill instead of the OS default.
- `apps/web-svelte/` `/identify` cascade review drawer is now the full editor at parity with the React `CascadeReviewDrawer`, replacing the condensed first-pass stub. Series results now render the full three-level cascade (series header with poster + backdrop `ImagePicker`s, per-season collapsible sections with their own poster picker + `FieldMaskGrid`, per-episode rows with expandable field masks + unmatched warnings + local-file hints), with a disambiguation `CandidatePicker` when the plugin returns multiple matches — picking a candidate re-runs the plugin with `{ externalIds: { tmdb } }` and re-mounts the drawer against the fresh scrape row. Movie and Episode payloads get dedicated compact review bodies with their own `ImagePicker`s (Still for episodes, Poster + Backdrop for movies) and typed field masks. Accept assembles a full `CascadeAcceptSpec` with per-season / per-episode overrides and posts to the matching `/video/{series,movies,episodes}/:id/accept-scrape` endpoint. New components under `src/lib/components/identify/`: `ImagePicker.svelte` (preview chip + modal with rank / resolution / language sort + language filter + "Use no image"), `CandidatePicker.svelte`, `FieldMaskGrid.svelte`, `EpisodeRow.svelte`, `SeasonSection.svelte`, `SeriesCascadeBody.svelte`, `MovieReviewBody.svelte`, `EpisodeReviewBody.svelte`.
- `apps/web-svelte/` `/plugins` now shows the **Auth OK** / **Auth Required** badge and the **Configure** / **Close** button for installed plugins that declare `authFields`. Expanding Configure reveals an inline auth panel that lists each auth field with its label, required marker, and "Get key →" link, a password input with a "(configured — enter new value to replace)" placeholder when auth is already stored, and a **Save Credentials** primary button that posts to `savePluginAuthKey` for each populated field. Matches the React plugin card layout.
- `apps/web-svelte/` `/jobs` has been rewritten into the full "Job Control" layout at parity with the React `JobDashboard`. Replaces the flat 5-column queue-card dump with: the "Job Control" header + description, the four-up overview stats row (Running, Backlog, Failures, Retained Done with schedule-interval hint), grouped queues (Library scans, Library maintenance, Video media pipeline, Metadata import, Gallery image pipeline, Audio pipeline, plus an Additional queues bucket for unknown queue names) using the shared `job-dashboard-queue-sections` grouping, styled `QueueCard`s with inline Run / Stop / Clear failures actions and the 4-up Running / Queued / Delayed / Errors metric grid, a Live Work section rendering `ActiveJobCard`s (status LED + force-rebuild badge + queue badge + status pill + progress meter + queued/started/trigger row + Kill task), a Failures section rendering expanded `FailedJobCard`s with ERROR OUTPUT pre blocks (maintenance jobs stay redacted in SFW mode), and a Recently Finished row list. Force-rebuild-previews is no longer surfaced on this page (it lives in /settings → Diagnostics, matching the React layout). New helper modules `src/lib/jobs/helpers.ts` and `src/lib/jobs/queue-sections.ts`, and six new components under `src/lib/components/jobs/`.
- `apps/web-svelte/` `/settings` is now a full interactive port of the React settings page, replacing the read-only debug dump. Covers every section at full parity: Watched Libraries with the inline folder browser + per-root enable / delete / scan-type / NSFW toggles, Content Visibility (three-way NSFW mode switcher wired to the Svelte `NsfwStore` + LAN auto-enable toggle + power-user tip block), Playback (Direct vs Adaptive HLS), Subtitles (auto-enable, preferred languages, display style picker, text size / vertical position / transparency sliders with a live `SubtitleCaptionOverlay` preview), Metadata Providers link card with live plugin count, Generation Pipeline grid (8 toggles + 3 number steppers + 2 quality sliders, plus the metadata-storage-dedicated confirm dialog that queues `migrate-video-asset-storage`), Generated Storage stats, and Diagnostics (force-rebuild previews + backfill pHashes). New primitives under `src/lib/components/settings/` — `ToggleCard.svelte`, `NumberStepper.svelte`, `QualitySlider.svelte`, `StorageStat.svelte`, `SubtitlesSection.svelte`, `DiagnosticsSection.svelte`, `WatchedLibrariesSection.svelte`. Every control auto-saves on commit via `updateLibrarySettings`.
- `apps/web-svelte/` NSFW toggle (⌘⇧Z) now refreshes grids in place. Previously `setMode`/`toggleShowOff` wrote the cookie but left the SvelteKit `+page.server.ts` data from the previous mode in place, so list pages (`/videos`, `/performers`, `/galleries`, etc.) kept showing the pre-toggle filtering until a manual reload. The store now calls `invalidateAll()` on every mode change (including the LAN auto-enable) so all server loaders re-run with the fresh cookie.
- `apps/web-svelte/` three runtime-breaking bugs fixed: `/studios` landing used `encodeURIComponent(studio.name)` in card links so clicking into a detail page 500'd against the ID-only `/studios/:id` API (now links by `studio.id`); `/tags/[id]` was passing the URL-encoded tag name to the API's ID endpoint and returning 500 (now falls back to a tag-list lookup by name, mirroring the React tag-detail page); and `/identify` + `/scrape` froze on load with `effect_update_depth_exceeded` because the row-rebuild `$effect` in `BulkScrape.svelte` read and wrote `videoRows`/`perfRows`/`studioRows`/`tagRows` in the same body (now the existing-row snapshots are read inside `untrack(...)` so writes don't re-trigger the effect).
- `apps/web-svelte/` `/plugins` page is now a full interactive port of the React version: 4 tabs (Installed, Obscura Community, Stash Community, StashBox Endpoints) with SFW-mode gating, stats row that flips between 2- and 4-column layouts, search + capability filter on the Installed tab, per-row toggle / remove / update / install actions for both Obscura plugins and Stash-compat scrapers, and full StashBox endpoint CRUD with inline test + enable/disable + preset-URL helpers. Plugin auth-field config is deferred to polish.
- `apps/web-svelte/` `/jobs` page now drives the queue: per-queue **Run / Cancel / Clear** buttons, per-row **cancel** on active jobs, global **Kill all** and **Acknowledge failures** and **Force rebuild previews** toolbar actions, 5-second poll, and inline success / error banners. Matches the React JobDashboard's interactive surface area.
...
- Svelte 5 primitives in `@obscura/ui-svelte`: `Button`, `Badge`, `Checkbox`, `StatusLed`, `Meter`, `Panel`, `MediaCard`. Variants, CVA classes, and trickplay scrub preview on `MediaCard` are preserved 1:1 from the React versions. Uses `@lucide/svelte` (Svelte 5-compatible) in place of `lucide-react`.
- `/design-language` parity page in the SvelteKit app for side-by-side visual checks against the Next.js `/design-language` on port 8008.
- SvelteKit app shell in `apps/web-svelte`: `+layout.server.ts` reads cookies + probes `/library/config` and `/system/status`, `+layout.svelte` wires all four context providers, `Sidebar`, `CanvasHeader`, `MobileNav`, `BreakingUpgradeGate`, and `LogoMark` (with NSFW-mode-aware ring). Dynamic sidebar-collapse cookie + NSFW ⌘⇧Z hotkey + Cmd+K palette are all live.
- Svelte 5 class-based stores in `apps/web-svelte/src/lib/stores/`: `NsfwStore`, `AppChromeStore`, `SearchStore`, `PlaylistStore`. Exposed via `provideX()` / `useX()` context helpers.
- Rune-based hook utilities in `apps/web-svelte/src/lib/hooks/`: `currentPath`, `elementInView` (IntersectionObserver), `selection`, `recentSearches` (localStorage-backed with multi-tab sync).
- API client layer for the SvelteKit app: `src/lib/api/{core,videos,entities,media,scrapers,library,system,types,video-query,phash-contributions}.ts` (client-side, uses `PUBLIC_API_URL`) and `src/lib/server/{core,videos,media,system}.ts` (server-side, uses `INTERNAL_API_URL`). Behavior-identical to the Next.js versions. Documented `depends()` / `invalidate()` conventions in `src/lib/server/README.md` — replaces Next.js `revalidateTag`, with keys like `videos`, `videos:{id}`, `galleries:{id}`, etc.
- Every top-level Obscura route is reachable in the SvelteKit app as a real SSR'd page, so the sidebar, breadcrumb nav, and mobile bottom nav fully work from day one. The `/tags`, `/tags/new`, and `/tags/[id]` routes are fully functional (read from `/tags`, create via the API, render aliases). The remaining entity routes (`/studios`, `/performers`, `/galleries`, `/images`, `/audio`, `/collections`, `/search`, `/videos`, `/identify`, `/resolve`, `/scrape`, `/scrapers`, `/jobs`, `/plugins`, `/settings`) are scaffolded with a `ComingSoon` panel that names the Linear issue tracking the deep port. `/` root shows a quick-nav grid of every section.
...
- SvelteKit `/collections/[id]/edit` is fully functional — 1:1 port of the React `CollectionEditorClient`: name/description form, Manual/Dynamic/Hybrid mode picker, recursive rule tree builder (`ConditionBuilder` + `ConditionGroup` + `ConditionRow`) with AND/OR/NOT group operators, per-entity-type filters (video / gallery / image / audio-track), all `CollectionRuleField` operators (equals, contains, between, in, …), autocompleting `ChipInput` for tags/performers/studios with "Add new" fallback, debounced live preview against the rule tree, Slideshow settings, and Collection Info sidebar.
...
- Actor detail Known For no longer repeats one card per episode when the same actor role appears across a Series. Repeated episode-only roles are collapsed into one Series-linked card, while one-off episode roles still link to the exact episode.
...
- The floating Svelte audio transport now waits for an explicit play action on `/audio/[id]` instead of auto-starting the first track when the page loads. Its fixed z-layer was also lowered so the sidebar and mobile shell sheets render above it again.
...
- `VideoPlayer.svelte` source-lifecycle effect no longer retriggers itself. The effect both read and wrote `streamMode` / `qualityMode`, which in Svelte 5 causes the effect to re-fire on every internal write — tearing the `<video>` element's `src` back down between `removeAttribute` and the next assignment, so `readyState` stayed pinned at `0` on the direct stream. The body is now wrapped in `untrack()` so only prop changes to `src` / `directSrc` / the element bind re-run it.
- SvelteKit typecheck is green end-to-end. Fixed a batch of pre-existing `svelte-check` errors: `playlist` store's entity-kind switch used `"audio_track"` instead of `"audio-track"`; the `/search` route used dashed `audio_library` / `audio_track` / `collection` kinds that never existed and referenced stale `name` / `thumbnailPath` / `studioName` fields (real shape is `title` / `imagePath` / `subtitle`); `/jobs` queue card referenced `queueName` / `queued` / `pending` (should be `name` / `waiting`); `/settings` storage block referenced `cacheBytes` / `thumbnailBytes` / `hlsBytes` / `databaseBytes` (real fields are `thumbnailsBytes` / `previewsBytes` / `trickplayBytes` / `totalBytes`); `/collections/new` used a made-up `"smart"` mode — it's now replaced with the full `CollectionEditor` for parity with the React `CollectionEditorClient`-with-`isNew`; `/performers` grouping closure was typed as `typeof data.performers` (an array) instead of its element; `BreakingUpgradeGate` had a `state` variable shadowing a Svelte store and broke `$state` typing.
- Video and Series cast now preserve saved character names all the way to the UI. Series detail responses expose `character`, video detail responses now label whether a displayed role came from the movie, the episode row, or inherited Series cast, and episode detail falls back to the Series character when the episode itself has no override.
- Series cast-strip role subtitles no longer hard-truncate after a single line. Long character names now wrap to two lines within the existing card width so the Cast & Crew rail stays readable without changing the card layout.
...
- Plugin-sourced video scrape results were saved with `entityType: "video"`, but `/scrapers/results/:id/accept` requires `"video_episode"` / `"video_movie"` and `/plugins/results/:id/accept` explicitly hands video accepts back to the scrapers route. The result was that Accept on the Identify page's Videos tab silently did nothing whenever the match came from a plugin. `/plugins/:id/execute` now probes `video_episodes` then `video_movies` for the target entity and persists the concrete kind, so the scrapers accept route can actually land the patch.
- The per-row Accept handlers on the Identify page (Videos, Video Series, Galleries, Images, Albums, Tracks) and the "Accept All" runners swallowed every error, leaving the row stuck on **Found** with no feedback. They now transition the row to the `error` status with the underlying message so the user can see what went wrong and retry.
...
- Video asset URLs and generated-asset maintenance jobs now use `videos` naming, so first-party routes no longer emit `/assets/scenes/...` or `scene-asset-layout`.
...
- Added `apps/web-svelte/src/lib/components/forms/README.md` documenting the boundary between domain-specific form controls (here) and generic UI primitives (in `@obscura/ui-svelte`).
- Narrowed the `@obscura/app-core` public surface. Internal SQL/upload primitives (`appearance-count-expressions`, `hierarchy`, `library-root-visibility`, `media-query-helpers`, `upload-utils`) are no longer re-exported from the package barrel — they're used only inside the package. No external callers were affected.
- Split the 875-line `apps/web-svelte/src/lib/identify/identify-runners.ts` into one runner per entity (`audio-library-runner.ts`, `audio-track-runner.ts`, `gallery-runner.ts`, `image-runner.ts`) plus a shared `runner-utils.ts` for the row-level plugin loop, helpers, and types. `BulkScrape.svelte` and the existing per-entity scrape runners now import from the new modules; behavior unchanged.
- Split `packages/app-core/src/video-core.ts` (2,186 lines): playback recording (`recordVideoPlayWrite`, `recordVideoOrgasmWrite`) extracted to `video-playback.ts`; thumbnail and preview management (`setCustomVideoThumbnail*`, `resetVideoThumbnailWrite`, `rebuildVideoPreviewWrite`, plus the internal `saveCustomVideoThumbnail` helper) extracted to `video-previews.ts`. Shared internals (`findVideoEntity`, `videoEntityTable`, `defaultEnqueueJob`) are now exported from video-core. Public package API unchanged.
- Split the 2,534-line `packages/app-core/src/media.ts` into four focused files: `gallery-media.ts` (galleries + images), `audio-libraries.ts`, `audio-tracks.ts`, and a small `media-shared.ts` holding the shared `findOrCreate*` helpers and `TxArg`. The monolith is gone; every `@obscura/app-core` export from the old file is preserved via the package barrel.
- `/api/changelog` now uses SvelteKit's `json()` helper and `RequestHandler` type on its error path instead of raw `Response.json()`, matching the rest of the API handlers.
...
- NSFW logic consolidated under `apps/web-svelte/src/lib/nsfw/` and `src/lib/components/nsfw/`. Utilities (`nsfw-cookie`, `nsfw-hotkey`, `nsfw-tags`), the rune store (previously `stores/nsfw.svelte`), the selector hook (previously `hooks/nsfw-aware-providers`), and the five `Nsfw*.svelte` components are now grouped by domain. Imports updated throughout; no behavior change.
- Actors, Studios, Tags, Galleries, Images, Audio, Collections, Videos, and Series now keep bulk selection and bulk actions in list mode only; grid mode remains the default browsing view. Grid/list choices on the grid-first entity pages are saved through the existing `ui_prefs` view preference records.
...
- `svelte-check` no longer fails on `video-stream.test.ts` when the mock DB object is cast to `AppDb`. The cast now goes through `unknown` first, matching TypeScript's expectation for shapes that don't sufficiently overlap with the target type. `pnpm turbo run lint typecheck` is green again.
- `@obscura/web-svelte` unit tests are green end-to-end again. The `/videos` and `/series` page-server tests were failing because `$env/dynamic/private` had no test mock; added `src/test/mocks/env-dynamic-private.ts` and wired it through `vitest.config.ts`. The `CollectionItemCard` tests were failing because the card now delegates to `VideoThumbnail`, which reads the NSFW store from context; wrapped the render in a new `CollectionItemCard.test-harness.svelte` that calls `provideNsfw`, following the pattern already used by `ImageLightbox.test-harness.svelte`.
...
- Gallery asset resolver (`/assets/galleries/[id]/cover`) now checks for a custom `cover-custom.jpg` before falling back to the linked `coverImageId`'s thumbnail. Image asset resolver (`/assets/images/[id]/thumb`) now checks for `thumb-custom.jpg` before `thumb.jpg`. When a gallery's cover resolves through an image, the nested image's `thumb-custom.jpg` is also preferred over `thumb.jpg`.
- Gallery and image search results now return `isNsfw` in their result meta, so thumbnails render with the correct blur state on `/search` and in the command palette without a second fetch.
...
- SvelteKit audio browsing got a substantial visual refresh. `/audio` grid cards now reveal a glowing brass play button on hover, scale the art slightly, show a small track-count chip over the cover, and animate a rotating disc on covers that haven't been scraped. `/audio/[id]` replaces the old track table with a new `TrackListRow` component — the active track gets a left brass accent bar, a four-bar equalizer animation while playing, an accent-gradient background, and hover play / rating / delete affordances. `/audio/tracks/[id]` was rebuilt around a full-width hero that uses the library cover as a blurred backdrop, a large dedicated cover square, oversized title, artist/album line, inline rating, and an action strip; details now live in a two-column grid with a sidebar that carries the library / studio links and a dedicated Technical readout.
...
- The dashboard, performer, studio, tag, and video detail/edit screens now use `video` / `series` naming all the way through their first-party React state and props, and the video detail screen now calls the live `/videos` play / orgasm / preview-rebuild routes directly instead of carrying dead `/scenes` fallback logic.
- Generated video asset helpers, worker processors, and cleanup jobs now use `videos` / `video-series` cache directory names and `video-asset-layout` identifiers instead of the old `scene` / `scene-folders` naming. Rebuilding previews or relocating generated files now lands everything under the same first-party terminology as the rest of the video stack.
- The bulk-scrape, resolve, and pHash contribution flows now pass `videoId` and render `video` rows end to end instead of carrying legacy `sceneId` / `scene` state through first-party web and API calls. That keeps the identify stack aligned with the rest of the Videos model while leaving upstream Stash/StashBox protocol terms untouched where they still matter.
- Worker/API media helpers and the remaining first-party resolve workflow state now use `video` naming too, including `/assets/videos/...` helper URLs, seed-media constants, cache-size accounting, and the unmatched-videos review queue. This removes another set of local `scene` aliases that no longer matched the underlying data model.
...
- Migrations run inside the API server on boot. The separate `node_modules/.bin/tsx src/db/migrate.ts` step in `infra/docker/entrypoint.sh` is gone; the API owns the lifecycle so the new breaking-upgrade gate (below) can pause boot before any schema mutation happens.
- "Folder" terminology is gone. The UI series browser is served from `/video-series` (was `/video-folders`), wire DTOs renamed `SceneFolder*Dto` → `VideoSeries*Dto` and `Scene*Dto` → `Video*Dto`, field names `childFolderCount` → `childSeasonCount`, `sceneFolderId/Title` → `videoSeriesId/Title`, `sceneCount` → `videoCount`, `direct/total/visibleSfwSceneCount` → `*VideoCount`. Legacy `/scenes` and `/scene-folders` route constants removed from `apiRoutes`. UI labels say "Series" / "Season" everywhere instead of "Folder" / "Subfolder".

### Removed

- Deleted the migration planning archive under `docs/superpowers/**` plus the old `docs/videos-to-series-rollout.md` runbook so the repo no longer carries the Svelte cutover or retired migration plans in its active docs tree.
...
- Video pages no longer get stuck on an endless “direct” source load when an MKV still needs an MP4 prep pass. The SvelteKit stream helper now ignores partial cache files, writes prepared MP4s atomically, drops embedded subtitle streams from the direct-play cache, and returns a quick retryable error so the player can switch to HLS instead of waiting on a long background conversion.
- ASS subtitle overlays no longer fail with `Failed to resolve module specifier 'jassub'` or silently miss the first boot because the `<video>` ref was not attached yet. The Svelte overlay now loads a prebundled JASSUB client from the static runtime assets and reboots whenever the actual player element becomes available, so saved subtitle tracks render without needing to open the Transcript tab first.
- Library scans no longer leave red failures in the Operations dashboard when they hit a corrupt video file. `probeVideoFile` now throws a dedicated `CorruptMediaError` when ffprobe reports "moov atom not found", "Invalid data found when processing input", or similar structural failures, and the `media-probe` processor catches it, logs a warning to the worker log, and completes the job normally. The affected video stays in the library with null technical metadata so you can still see it in the UI and delete it if you want, but the job run is no longer flagged as failed each scan.
- Perceptual-hash generation no longer fails an entire `fingerprint` job when a single video frame is unreadable. The bundled `obscura-phash` helper now retries each sprite frame at small time offsets (±0.25s, ±1.0s) before giving up, and falls back to substituting an adjacent successful frame so the 5×5 montage is still well-formed. If the helper still can't decode anything — a truly corrupt file — the worker logs "Skipping phash for …" and finishes the fingerprint job with MD5 and oshash saved, instead of aborting the whole job.
- Gallery scans no longer fail with `Failed query: delete from "galleries" where "galleries"."id" in (…)` when a parent folder is pruned but its children are still in the database. The self-referential `galleries.parent_id` FK now uses `ON DELETE SET NULL` (migration 0019) so deleting a stale parent gallery just orphans its children instead of blocking the cleanup pass. The next scan re-parents or reaps the orphans normally.
- Integration tests in `tests/api.integration.test.ts` and `tests/worker.integration.test.ts` were still importing the deleted `scenes`, `scene_markers`, and `scene_subtitles` tables from the pre-0.20 schema and crashed before running any assertion. Rewrote both suites against the current `video_movies`, `video_markers`, and `video_subtitles` tables and the `/videos/*` routes, so the integration tier actually exercises the API and worker again.
- Added integration coverage for routes that previously had none: `POST/GET/PATCH/DELETE /performers`, `/tags`, `/studios` (including studio find-or-create idempotence), `/video-series` list/detail/update (including studio auto-create on update), `/jobs` dashboard, `POST /jobs/queues/:name/run` for `preview`, and `/jobs/cancel-all`. This closes a gap where most API CRUD had no automated regression protection.
- Added test coverage for `packages/db/src/lib/`: `propagateEpisodeNsfw` / `propagateMovieNsfw` (library-root override, tag signal, performer signal, studio inheritance, clear-when-no-signals) and `pruneUntrackedLibraryReferences` (missing-file cleanup, empty-series/season cascade, zero-enabled-video-roots safety rail, orphans-outside-root removal). These functions had zero automated coverage even though a bug in either would silently corrupt or wipe library data.
- Added integration coverage for `evaluateRuleTree` / `previewRuleTree` in the collection rule engine: scalar equality, AND across fields, OR across fields, NOT inversion, tag-name relation, studio-name-in, performer-name-in, and preview counts. Dynamic/hybrid collection membership silently drifting was a previously untestable failure mode.
- Added unit coverage for `packages/stash-import` (whitespace / HTML-entity / date / image-url normalization, performer field mapping, case-insensitive name dedup) and for `resolveActionDef` in the YAML parser (url-pattern matching, generic fallback, first-valid selection, null on missing/invalid action).
- Series poster and backdrop rendered as broken images after accepting a cascade. `scrape-accept.service.ts` was writing `/assets/video-folders/:id/cover` and `/backdrop` into `videoSeries.posterPath` / `backdropPath`, but those URLs were renamed to `/assets/video-series/:id/…` during the folder → series terminology pass and the old route paths had no handler, so the API returned 404 for every accepted series image. The accept path now writes the canonical `/assets/video-series/:id/…` URLs, and a pair of legacy alias routes in `apps/api/src/routes/assets.ts` resolve `/assets/video-folders/:id/cover` and `/backdrop` to the same on-disk files so any series already accepted against the broken URL starts rendering again without a migration.
- Applying a plugin cascade (Identify → Layers → Apply cascade) no longer times out with a 504 from nginx when the series has many seasons or slow image sources. Nginx's `/api/` and `/assets/` locations now use a 600s `proxy_read_timeout` / `proxy_send_timeout` (up from the default 60s), the series-level poster/backdrop/logo downloads run in parallel via `Promise.all`, and each individual image fetch has a 20s `AbortSignal` timeout so one hung TMDB URL can't stall the whole accept.
- Saving a plugin API key (e.g. the TMDB API key) on the unified Docker image no longer crashes with `OBSCURA_SECRET environment variable is required for plugin credential encryption`. The entrypoint now generates a random `OBSCURA_SECRET` on first boot and persists it to `/data/.obscura-secret` (0600) so plugin credentials encrypt/decrypt across container recreations without the user having to provision the variable by hand. An explicit `OBSCURA_SECRET` env var still wins if set.
- TMDB plugin cast was never being saved to the Actors library on single-movie or single-TV identifies. The plugin emitted a legacy `performerNames: string[]` field that the new typed accept pipeline (which reads `cast: NormalizedCastMember[]`) ignored, so accepting a scrape left zero rows in `performers` / `video_movie_performers` / `video_episode_performers`. The TMDB plugin now emits the normalized `cast` shape for both paths (and the TV call appends `credits` so the series-level cast is actually fetched). Update to TMDB plugin v0.3.1 to pick up the fix.
- `upsertPerformerByName` in the scrape-accept pipeline now matches case-insensitively and trims whitespace, so "Tom Hanks" and "tom hanks" no longer produce duplicate Actor rows.
...
- **Corrupt video files no longer spam the Operations dashboard with failed jobs.** If a file on disk is truncated or missing its MP4 header ("moov atom not found"), the media-probe and fingerprint steps now log a warning and move on instead of marking the task as failed. The file stays in your library with empty technical metadata so you can find it in the UI and clean it up; re-scans won't keep flagging it red.
- **Perceptual hashing is more forgiving on slightly damaged videos.** A single unreadable frame in the middle of an otherwise-fine video used to fail the whole fingerprint job. The hasher now retries each sprite frame at small time offsets and substitutes an adjacent good frame when it needs to, so videos with occasional glitchy frames get a real phash instead of a red X.
- **Gallery scans no longer fail when an image folder is removed between scans.** Previously, if you deleted a folder on disk that had subfolders Obscura still knew about, the cleanup pass would crash on a foreign-key error. Deleting a parent gallery now correctly orphans its children, which the next scan either re-parents or cleans up.
- **Saving plugin API keys no longer crashes on the unified Docker image.** Previously the TMDB plugin (and any other plugin with credentials) returned a 500 when you tried to save an API key because the encryption secret wasn't set in production. Obscura now generates and persists its own plugin-credential secret in the data volume on first boot, so plugin keys round-trip without you needing to set anything. You can still provide an explicit `OBSCURA_SECRET` environment variable if you want to manage it externally.
- **Series poster + backdrop show up after a cascade accept.** Previously the Apply-cascade flow downloaded the images to disk correctly but saved a stale URL into the database, so the series card and detail page continued to show empty art after a successful apply. New accepts write the canonical URL, and any series you already accepted in the broken state will start rendering again automatically.
- **Applying a cascade no longer times out.** Pressing "Apply cascade" in the Identify review drawer on a series with many seasons or slow image sources used to 504 after 60 seconds. The API proxy now tolerates accepts up to 10 minutes, the series poster / backdrop / logo download in parallel, and any single image that hangs is abandoned after 20 seconds so it can't drag down the rest of the apply.
- **The Videos page opens to Series view by default.** Instead of landing on a wall of every individual file you own, the page now opens onto the series browser (shows + seasons + movies), matching how most users actually think about their library. Your last-picked view is still remembered, so if you prefer the flat grid or compact list, toggling once sticks. Existing users who had already picked a view are unaffected.
- **Plugin-provided cast is now saved to your Actors library.** When a community plugin (TMDB, TVDB, etc.) returns a movie or TV cast list, accepting the scrape creates real entries in the Actors table and links them to the video, instead of silently dropping the data. Matching is now case-insensitive, so "Tom Hanks" and "tom hanks" collapse to the same actor row rather than duplicating. **To pick up the TMDB-side fix for missing cast on single-movie / single-TV identifies, reinstall or update the TMDB plugin to v0.3.1** (use the new "Check for updates" button in Settings → Plugins).
- **Community plugins update themselves now.** Open Settings → Plugins → Installed and click **Check for updates** — Obscura compares every installed community plugin against the public index at [obscura-community-plugins](https://github.com/pauljoda/obscura-community-plugins) and surfaces an **Update available** badge with a one-click **Update** button for each plugin that has a newer version. Production installs default to the public GitHub-hosted registry out of the box; developers can still point `OBSCURA_PLUGIN_INDEX_PATH` at a local clone to iterate on plugin source.
- **Scenes → Videos is now the only model.** The `scenes` / `scene_folders` tables and their joins are gone; the `videos` → `video_series` → `video_seasons` → `video_episodes` / `video_movies` shape is the one true video model. If you are upgrading from a build that still used the old scenes tables and had data in them, Obscura will greet you on first boot with a one-time upgrade prompt explaining that the legacy rows will be dropped and your library will need a rescan — your video files on disk are untouched, only the database rows are rebuilt. After you click through, future updates are very unlikely to require this kind of break again.
- **Folders are now Series (and Seasons).** The word "folder" is gone from the UI wherever it referred to a grouped set of videos. The series browser, filter chips, metadata panels, search result groups, and identify tabs all say "Series" now. Subfolder counts on series cards say "3 seasons" instead of "3 subfolders".
- **Actor detail pages now show a Series section.** If an actor appears in any series (via `video_series_performers`), those series render as a row of cards above the individual videos — same card style studios and tags already used. Clicking a series card drills into the videos page scoped to that series, just like clicking from the library browser.
...
- **Cascade review picks real TMDB image candidates.** The Movie Database community plugin now queries `/tv/{id}/images` and `/tv/{id}/season/{n}/images` in addition to the base detail response, so the series poster / backdrop / logo pickers and season poster pickers open onto **every** candidate TMDB has rather than just one. Each candidate carries language, width, height, and TMDB's vote_average as a rank hint, so the picker's sort-by-rank default surfaces the most-voted art first. Requires TMDB plugin v0.3.0.
- **TMDB search is strictly tighter and returns multiple matches.** Queries are normalized before hitting TMDB (stripping trailing `!`, apostrophes, and stray punctuation that used to turn the search into a fuzzy wildcard), results are re-scored with a token-set similarity against the original title, and the top 10 are returned as `NormalizedSeriesCandidate[]` for the cascade review drawer to render as a disambiguation picker. Picking a candidate re-runs the plugin with `externalIds: { tmdb }` and replaces the drawer's cascade in place. Fixes "Blue's Clues and You!" landing on "Blue's Clues and You! Nursery Rhymes" with no correction path.
- **NSFW plugins and scrapers are hidden in SFW mode.** The identify button, the bulk identify / scrape flow, and the plugin popover on detail pages all funnel their installed-plugin lists through `useNsfwAwareProviders`, which drops any entry whose `isNsfw` flag is true when the NsfwContext is in `off` mode. A user browsing SFW will never see an NSFW provider as a seek option.
- **Cascade accept downloads all images to local disk.** Accepting a series cascade now downloads the selected poster, backdrop, and logo for the series and per-season posters to the local cache directory, making the entire library offline-capable. The asset routes serve the downloaded files for both the manual-upload path (`cover-custom.jpg`) and the scrape-accept path (`poster.jpg`, `backdrop.jpg`). Season posters go to `/assets/seasons/:id/poster` via a new asset route. Failure to download any single image is non-fatal — the accept completes and the slot stays null.
- **Episode number badge on video cards.** Video grid cards now show a compact **S01E03** badge in the top-left corner of the thumbnail when the underlying row has a season and episode number. Visible in both the flat video grid and the season drilldown view, so the user can immediately confirm episode ordering at a glance.
- **Browser back works between season drilldown levels.** Clicking a season card now pushes `&season=N` to the URL via `history.pushState` so browser back returns to the season grid instead of jumping to the root videos page. A `popstate` listener restores the season number from the URL on browser-back. The "← All seasons" link pops the param. Initial page load with `?folder=...&season=N` deep-links directly into the season's episode list.
- **Cast + guest stars render in the cascade drawer.** The series header now shows the top 20 cast members (with character names) below the genre chips. Each cast chip is differentiated as "existing actor" (default styling) or "will create new actor" (accent styling + `NEW` badge) by checking against the performer list on mount. Per-episode guest stars are now written to `video_episode_performers` during cascade accept via a new `epMask.cast` branch.
- **Genres in cascade drawer show existing vs new.** Genre chips are now styled with accent + `NEW` badge for tags that don't yet exist in the system, and default styling for tags that already do. Same differentiation on cast/performer chips.
- **Header backdrop blur reduced.** The poster-as-backdrop fallback now uses `blur-lg` (12px) instead of `blur-2xl` (40px) and slightly higher opacity for a sharper, more visually readable header when no dedicated backdrop image exists.
- **Detail-page identify now passes `localSeasons`.** The per-entity `IdentifyButton` (on the series hero) and the cascade review drawer's candidate re-run both fetch `/video/series/:id` up front when `entityKind === "video_series"` and feed the on-disk season/episode tree into the plugin input as `localSeasons`. Without this the cascade drawer rendered "The plugin did not return any season data" even though the plugin succeeded at the series level. Chair Company (1 season, 8 episodes) now round-trips through the drawer with real episode rows, match status, and the per-season poster picker populated.
- **Identify button on detail pages.** Spec §9.4 entry points landed as one shared `IdentifyButton` component: **Identify Series** on the series folder hero (`/videos?folder=…`), and **Identify** / **Re-identify** on the video detail page (`/videos/:id`) that automatically routes to the right plugin action based on whether the row is a movie or an episode. Click → popover of enabled plugins → runs the seek → opens the same cascade review drawer the Identify tab uses. No more bulk-only identify from the UI.
- **Deleting a video now cleans up its on-disk derivatives.** `DELETE /videos/:id` removes the DB row, then unlinks every thumbnail, card thumbnail, preview clip, sprite map, and trickplay VTT for that video (both dedicated-dir and sidecar layouts). When `?deleteFile=true` is set, the source file is unlinked too. Before this, the row went away and the cache files leaked until the next library maintenance pass.
- **Identify + TMDB now fill the cascade review drawer.** Folder seeks persist a typed `NormalizedSeriesResult` on each saved scrape row (seasons and episodes matched from your library plus TMDb), so **Layers** opens with real season/episode rows instead of an empty payload. Re-install or refresh the **The Movie Database** community plugin (v0.2.0) to pick up the new `folderByName` behavior.
- **Cascade review drawer for the Identify tab.** Folders that come back with a typed `proposedResult` from a seek now show a **Layers** action in their row; clicking it opens a full-height review drawer with three modes: series (header + poster/backdrop/logo image pickers, collapsible season sections with their own poster pickers, per-episode rows with match status and per-field checkboxes), movie (compact header + image pickers + per-field checkboxes), and episode (compact still picker + per-field checkboxes). A generic **ImagePicker** component (`apps/web/src/components/identify/image-picker.tsx`) drives every image slot: click a thumbnail to open a modal grid of all `ImageCandidate[]` at preview resolution, sort by rank / resolution / language, filter by language or "hide language-agnostic", or pick "Use no image". Selections live on the drawer's local state and get serialized into the `CascadeAcceptSpec` payload sent to the new typed accept endpoints (`/video/{movies,series,episodes}/:id/accept-scrape`). An "Accept all" shortcut flips every season + episode + field mask on at once for users who have reviewed and want to bulk-land the result.
- **Seasons are now a first-class layer in the Videos UI.** Series folders that have numbered seasons render a new **Seasons** grid before the video list — click a season poster to drill into just that season's episodes, with a "← All seasons" shortcut back out. Flat series (Specials-only / season 0 only) continue to show the video grid directly, so single-season movies and standalone releases are unaffected. Backed by a new `/video-folders/:id` response field (`seasons[]` + `renderingMode`) and a `seasonNumber` filter on `/videos`.
- **Exhaustive terminology + rename pass — "wipe scenes from history as though it never happened".** The folder view of the Videos page previously read "Scene folders" / "Uncategorized scenes" / "8 scenes"; it now reads **Series**, **Movies**, and **8 videos** consistently. Every filter chip, settings description, dialog title, search placeholder, tab label, breadcrumb, and empty-state that said "scenes" now says "videos". The component tree itself has been renamed to match: `components/scenes/` → `components/videos/`, `scene-detail` → `video-detail`, `scene-edit` → `video-edit`, `scenes-page-client` → `videos-page-client`, and the whole set of `Scene*` React components (`SceneCard`, `SceneDetail`, `SceneEdit`, `SceneGrid`, `SceneFileInfo`, `SceneMarkerEditor`, `SceneMetadataPanel`, `SceneTranscriptPanel`, `ScenesPageClient`, `ScenesListPrefs`) became `Video*` / `Videos*`. The contract DTO names in `@obscura/contracts` stay as `SceneListItemDto` / `SceneDetailDto` etc. because they're the wire shape the API still serves, but the web side now imports them under `VideoListItem` / `VideoDetail` / `VideoStats` aliases (legacy `Scene*` aliases kept alongside for backward compat).
...
- **Custom folder names.** Renaming a series folder from the UI now sticks — `video_series` carries a `custom_name` column, and the folder cards / detail pages show the custom name when it's set. The migration copies any existing `scene_folders.custom_name` values onto their new `video_series` rows. The plugin identify flow also lands the scraped title on `custom_name` so your on-disk folder structure is preserved.
- **Video folder cover / backdrop upload.** You can now replace a series' poster or backdrop from the folder edit drawer — drop an image, it goes to `/assets/video-folders/:id/cover` (or `/backdrop`), and the card updates immediately. Scraper-sourced images use the same slot; deleting a manual upload falls back to whatever the last scrape wrote.
- **Plugin scrape-accept downloads poster images.** Accepting a folder-level scrape (e.g. TMDb identify) now downloads the proposed poster to local disk and wires it up as the folder cover. Before this, plugin accepts only wrote text metadata and left the image slot empty.
- **Folder-scoped video uploads.** Uploading a video from inside a series detail page now places the file in the series' folder and creates an episode row, instead of silently rejecting or dumping the file at the library root. The old library-root upload flow still works for brand-new movies.
...
- **`SceneDetailDto.seasonNumber` / `episodeNumber` / `absoluteEpisodeNumber`** — previously only `episodeNumber` was on the list DTO and missing from the detail DTO entirely. `getVideoSceneDetail()` in `video-scene.service.ts` now selects and emits all three for video_episode rows; movies emit all three as `null`. `UpdateVideoBody` accepts the three as optional writes (ignored on movie rows so the edit form doesn't need to fork per-kind).
- **`"episode"` sort option on the video list.** New `SortOption` enum value (`apps/web/src/lib/video-browse-types.ts`), surfaced in `filter-bar.tsx` as "Episode order", default direction `asc`. `parseSort` in `video-scene.service.ts` turns the key into a compound ORDER BY `(seasonNumber, episodeNumber, absoluteEpisodeNumber, createdAt)` ascending for episode queries; movie queries fall through to a stable createdAt tiebreaker so the sort key is well-defined even on mixed result sets. `videos-page-client.tsx` forces `sortBy = "episode", sortDir = "asc"` whenever `initialActiveFolder` is set so entering a series folder always shows the episode-ordered list, and reverts to the user's saved preference on exit.
- **`VideoMetadataPanel` + edit form episode display.** The detail page's metadata panel adds an **Episode** `InfoRow` showing `S01E03` / `Specials E02` / `Abs 27` formats when the underlying row carries the fields, and the **Metadata** edit tab grows Season / Episode / Absolute Episode number inputs rendered only for `entityKind === "video_episode"`. Non-edit mode also shows a matching `MetadataRow` labeled "Episode" with a TV icon.
- **Plugin re-run on candidate pick.** `CascadeReviewDrawer` now owns a `currentScrapeResultId` state and a `reRunWithExternalId(tmdbId)` helper that calls `executePlugin` with `{ externalIds: { tmdb } }`, persists a new scrape row on the server, and swaps the drawer body in place (keyed on the new scrape-result id so React re-mounts the cascade body and resets local state). The series cascade body threads `onPickCandidate` down to `CandidatePicker`, which prefers `externalIds.tmdb` when extracting the id so the bypass path matches.
- **`IdentifyButton` component** (`apps/web/src/components/identify/identify-button.tsx`) — a generic per-entity identify entry point used on both the series folder hero and the video detail page. Lazily fetches enabled plugins, filters by capability (`seriesCascade` / `seriesByName` / `folderByName` for series, `movieByName` / `videoByName` for movies, `episodeByName` / `episodeByFragment` / `videoByName` for episodes), runs `executePlugin` against the chosen plugin with `saveResult: true`, then opens the existing `CascadeReviewDrawer` keyed on the resulting scrape-result id. Wired into `video-detail.tsx` (as an **Identify** / **Re-identify** button next to the Organized toggle, driven by the new `scene.entityKind` field) and into `videos-page-client.tsx`'s series hero (as **Identify Series**).
- **`SceneDetailDto.entityKind`** — new required discriminator (`"video_episode" | "video_movie"`) on the video detail payload so the UI can tell episodes and movies apart in a single request. Emitted by `getVideoSceneDetail()` based on the underlying row's table.
- **`GET /video/series/:id` episode `filePath` in API responses** — the series detail payload now includes each episode’s on-disk path so the Identify tab can pass `localSeasons` into TMDB `folderByName` for per-file matching in the cascade UI.
- **`deriveProposedResultFromPluginOutput()`** in `apps/api/src/lib/plugin-proposed-result.ts`, wired from `POST /plugins/:id/execute` whenever `saveResult` persists a scrape row.
- **Cascade review drawer + generic ImagePicker.** Two new files under `apps/web/src/components/identify/`: `image-picker.tsx` (a controlled `ImageCandidate[]` picker with thumbnail chip + modal grid, sort by rank / resolution / language, language filter + hide-agnostic toggle, "Use no image" opt-out, and poster/backdrop/still/logo aspect presets) and `cascade-review-drawer.tsx` (a full-height drawer that discriminates on `scrape_results.proposedResult` shape and renders the series cascade, compact movie review, or compact episode review as appropriate). The series mode draws a header with poster/backdrop/logo pickers, optional disambiguation candidates strip, collapsible season sections with their own poster pickers, per-episode rows with match status badges and per-field checkboxes, and an "Accept all" shortcut plus an "Apply cascade" footer that POSTs the assembled `CascadeAcceptSpec` to `/video/series/:id/accept-scrape`. Movie and episode modes POST to the compact `/video/movies/:id/accept-scrape` and `/video/episodes/:id/accept-scrape` endpoints. `identify-video-folders-tab.tsx` grows a new **Layers** action button that opens the drawer for any found row with a scrape-result id.
- **`GET /scrapers/results/:id`** — fetch a single scrape result row by id, used by the cascade review drawer to hydrate its typed `proposedResult` payload without pulling a 500-row list page. Paired client helper `fetchScrapeResult(id)` in `apps/web/src/lib/api/scrapers.ts`.
- **Typed `proposedResult` + `cascadeParentId` on `ScrapeResultDto`.** The web DTO now surfaces the two `scrape_results` columns from migration 0011: `proposedResult` (the `NormalizedMovieResult` / `NormalizedSeriesResult` / `NormalizedEpisodeResult` discriminated-union payload) and `cascadeParentId` (threads child scrape-results back to a parent cascade row). Paired client helpers `acceptVideoMovieScrape`, `acceptVideoEpisodeScrape`, and `acceptVideoSeriesScrape` targeting the typed accept endpoints with `{ fieldMask, selectedImages, cascade }` bodies, plus exported `AcceptFieldMask` / `SelectedImages` / `CascadeAcceptSpec` types for the drawer to consume.
- **Seasons grid on series detail pages.** New `VideoSeriesSeasonDto` in `@obscura/contracts` (`seasonNumber`, `title`, `posterPath`, `episodeCount`, `previewThumbnailPath`) and a `seasons[]` + `renderingMode: "flat" | "seasons"` pair on `SceneFolderDetailDto`. `video-folder.service.ts` adds `fetchSeriesSeasons(seriesId)` which counts episodes per season, grabs a preview thumbnail from the season's first episode, filters out empty seasons, and returns `"flat"` when the only non-empty season is number 0 (Specials-only). `/videos` accepts a new `seasonNumber` query param that joins on `video_episodes.season_number` when filtering inside a series folder. `videos-page-client.tsx` consumes both — it shows a clickable poster grid for multi-season series, drills into a single season's episode list on click, and renders a "← All seasons" back shortcut as the video-section action.
...
- **Video folder cover/backdrop upload routes** at `POST /video-folders/:id/cover` and `/backdrop` (plus DELETE), backed by new `uploadVideoFolderCover`, `setVideoFolderCoverFromUrl`, and `deleteVideoFolderCover` helpers in `video-folder.service.ts`. Images live in the same `getGeneratedSceneFolderDir(seriesId)` cache as before, so scene-folder URLs and video-folder URLs are interchangeable.
- **Asset GET routes** at `/assets/video-folders/:id/cover` and `/backdrop` — new video-folders URL shape served from the shared cache dir; the legacy `/assets/scene-folders/:id/*` aliases are preserved.
- **`uploadVideoEpisode(seriesId, file)`** service + `POST /videos/upload` accepts a `seriesId` multipart field. When set, the file is written under `video_series.folder_path`, a season-0 (Specials) row is created on demand, and a `video_episodes` row points at the new file. Downstream probe/fingerprint/preview jobs get enqueued the same way the library scan does.
- **Videos-to-series stage() copies subtitles, markers, custom folder names, and collection_items.** Legacy `scene_subtitles` rows land in `video_subtitles`, `scene_markers` land in `video_markers`, `scene_folders.custom_name` merges onto `video_series.custom_name`, and `collection_items.entity_id` gets rewritten to the new video entity uuid (the `entityType = "scene"` tag stays for wire compat — the hydration service resolves against the new tables under the same type). Metrics now report `subtitlesCopied`, `markersCopied`, and `collectionItemsRewritten`.

### Removed

- **Orphan `/video-library` route.** A non-linked read-only stub from Plan D3 that listed series and movies in a debug-grade grid; the real surface is `/videos`. Deletes both the page file and the `lib/api/video-library.ts` client helpers. The backend read routes (`/video/library/counts`, `/video/movies`, `/video/series`) stay registered but dormant — nothing calls them in the web app.
- **`useLibraryRootAsFolder` setting retired.** The videos-to-series model gives seasons first-class status, so the library root is never treated as a folder anywhere in the app. Drizzle migration `0016_perpetual_falcon` drops `library_settings.use_library_root_as_folder` (with `IF EXISTS` for post-finalize installs), the setting is removed from the contract DTO and the settings UI toggle, and the audio + gallery scan processors hardcode the "folder starts at the first subdirectory under the root" depth rule. `libraryContainerTitle` and `hierarchyFolderDepth` drop their boolean arg; tests updated. `LEGACY_SCHEMA_SENTINELS` gets a matching `0016_perpetual_falcon` entry.

### Fixed

- Several SvelteKit settings, jobs, and shared editor surfaces now keep their local state in sync with refreshed route data instead of capturing stale initial values. This removes a large chunk of the Svelte warning backlog and fixes a handful of subtle cases where labels, sliders, time fields, and loaded settings could drift out of sync after reloads or saves.
- The migration E2E harness no longer assumes the old container-only fixture path or the retired `/scenes` surface. Playwright now reuses an existing enabled video library when possible, falls back to the checked-out repo fixtures when available, exercises `/videos` and `/videos/:id` in smoke coverage, and keeps the video-detail parity snapshots keyed to `/videos/[fixtureId]` instead of a transient UUID.
- The restored SvelteKit shell controls now expose stable accessibility metadata. The command palette and mobile overflow sheet identify themselves as modal dialogs, the mobile nav and overflow links publish `aria-current`, the playlist queue toggles announce expanded state and control targets, and the shell providers read their initial SSR values through callbacks so the Svelte 5 context stores do not capture stale layout data on boot.
- **Studio detail page crashed with 500 in SFW mode.** The `getStudioById` SQL used an unqualified `is_nsfw` column reference in a JOIN between `video_episodes` and `video_series`, causing PostgreSQL to reject the query as ambiguous when `?nsfw=off`. The blanket `catch` block in the web page then rendered "Studio not found" instead of the real error. Fixed by qualifying the NSFW clause with `ve.is_nsfw` for episodes and `vm.is_nsfw` for movies.
- **Season/episode counts excluded NULL `is_nsfw` rows in SFW mode.** Three queries in `video-folder.service.ts` (`fetchSeriesSeasons`, `fetchSeriesEpisodeCounts`, `fetchSeriesPreviewThumbnails`) used `eq(isNsfw, false)` instead of `ne(isNsfw, true)`, which excludes rows where `is_nsfw IS NULL`. Episodes scanned before the NSFW flag was populated would silently vanish from season grids. Changed to `ne(isNsfw, true)` to match the rest of the codebase.
- **Tag update response always returned `sceneCount: 0`.** `updateTag` hardcoded the count instead of computing it via `tagTotalSceneCountExpr()`. Any UI component that refreshed from the PATCH response showed 0 videos for the tag until the next full reload.
- **Search result badges showed raw entity kinds.** Fallback search result cards rendered `item.kind` directly (e.g. "scene-folder") instead of the human-readable label from `SEARCH_KIND_CONFIG` (e.g. "Folders"). Both the default and compact card variants now display the configured label.
- **TMDB series identify showed only one poster / backdrop candidate.** The plugin's `imgCand()` helper wrapped a single URL in a one-element array and never called TMDB's dedicated `/tv/{id}/images` / `/tv/{id}/season/{n}/images` endpoints, so the cascade drawer's poster and backdrop pickers only had a single option to choose. Plugin v0.3.0 fetches both image endpoints upfront (with `include_image_language=en,null` so English + language-agnostic art always show up), normalizes the `posters[]` / `backdrops[]` / `logos[]` arrays into `ImageCandidate[]` with width / height / language / vote_average-as-rank, and falls back to the single-URL paths on the detail response if the `/images` call fails. Season posters get the same treatment via `/tv/{id}/season/{n}/images`. Requires re-installing or refreshing the TMDB plugin.
- **TMDB folder identify landed on the wrong series with no correction path.** `folderByName` took TMDB's first search result unconditionally and returned no `candidates[]` list, so "Blue's Clues and You!" landed on "Blue's Clues and You! Nursery Rhymes" and the cascade drawer had nothing to disambiguate against. Fixed by (1) normalizing the query before hitting `/search/tv` (strip trailing bangs, apostrophes, punctuation that TMDB's scorer treats as fuzzy wildcards), (2) re-scoring the top 20 results with a token-set similarity against the original title so exact matches float to the top, (3) returning the top 10 as `NormalizedSeriesCandidate[]` so the review drawer shows a disambiguation picker when the search is ambiguous, and (4) accepting `externalIds.tmdb` as an input so the review drawer can re-run the plugin with a specific id when the user picks a candidate.
- **Cascade review candidate picker did nothing on click.** The picker previously just stored the chosen id in local state and rendered a "re-run with this id" hint. It now calls back through `onPickCandidate` into the drawer shell, which calls `executePlugin(pluginPackageId, action, { externalIds: { tmdb } })` with `saveResult: true`, persists a fresh scrape row, and points the drawer at the new id — React re-mounts the cascade body via a `key={currentScrapeResultId}` on the body component so all local state resets cleanly.
- **NSFW plugins leaked into SFW identify runs.** The installed-plugin lists in `identify-button.tsx` (per-entity identify on detail pages) and `bulk-scrape.tsx` (the `/identify` page) both filtered the fetched list by `enabled` only, never by `isNsfw` — so an NSFW plugin was offered as a seek source even when the user was browsing in SFW mode. Both now route through `useNsfwAwareProviders`, which drops NSFW rows when `NsfwContext.mode === "off"`. The same hook also filters the stash-compat scraper package list (`ScraperPackageDto.isNsfw`).
- **`deleteVideoScene` leaked on-disk cache files.** The function previously dropped the DB row but never unlinked the generated derivatives under `$cache/video-{movies,episodes}/:id/…` or `$cache/videos/:id/sidecar/…`. The new implementation reads the `file_path` before deletion, then best-effort unlinks every path returned by `allSceneVideoGeneratedDiskPaths`. When `deleteFile=true`, the source file is unlinked too. Both unlinks are wrapped in try/catch so a stale inode never blocks the DB delete.
- **Plugin identify rows never populated `scrape_results.proposed_result`.** Saved plugin seeks (including TMDB folder identify) now derive and store a normalized movie / series / episode payload via `@obscura/plugins` normalizers, so the cascade review drawer and typed `/video/series/:id/accept-scrape` path receive the metadata they expect. Legacy `proposed_*` columns are still filled for quick accept and the old identify preview.
- **Catastrophic data-loss bug in `pruneUntrackedLibraryReferences`.** Migration 0010 added `library_roots.scan_movies` and `scan_series` with `DEFAULT false` and never backfilled from the legacy `scan_videos` column, so every existing library root ended up with both new toggles off. The prune helper — called before every library scan — filtered roots by `scanMovies || scanSeries`, ended up with an empty video-root list, and then deleted every `video_episodes` / `video_movies` row as "orphaned". This was the reason a fresh `/videos` view was empty after running the migration on a populated install.
  - `pruneUntrackedLibraryReferences` now **skips the orphan-delete step entirely** when there are no video-enabled roots. Missing-file pruning still runs because those deletions are tied to actual disk state. A misconfiguration can no longer empty the library in a single call.
  - Drizzle migration `0015_next_pride` flips the defaults on both `scan_movies` and `scan_series` to `TRUE` for new roots, and backfills any existing root that still has both toggles off (honoring the pre-0014 `scan_videos` flag where the column still exists, otherwise leaving exclusively image / audio roots alone).
  - The legacy-install bridge (`reconcileSchema`) runs a matching one-shot backfill on installs that still have `library_roots.scan_videos`.
- Studios, performers, and tags list queries no longer fail with `column reference "id" is ambiguous` after finalize. The computed scene-count SQL expressions now use fully-qualified raw references (`"studios"."id"`, `"performers"."id"`, `"tags"."id"`) in their correlated subqueries so PostgreSQL resolves the outer column against the intended table instead of the inner-scope `video_episodes.id` / `video_movies.id`.
- `library_roots.scan_videos` is now dropped by drizzle migration `0014_majestic_zaran` (with `IF EXISTS`) instead of inside `finalize()`. The previous inline drop in finalize left the drizzle schema and the live DB out of sync on installs that finalized before the schema removed the column, so the `settings/library` query crashed with `column "scan_videos" does not exist`. The same migration also retires the cached `performers.scene_count` / `tags.scene_count` / `studios.scene_count` counter columns — all four column drops run from a single migration file that is idempotent, so pre-finalize, post-finalize, and fresh installs all land on the same schema.
- `reconcileSchema` in `apps/api/src/db/migrate.ts` now checks whether `scene_subtitles`, `scene_markers`, `scene_folders`, and `scenes` still exist before re-applying their pre-baseline deltas. On installs that finalized the videos-to-series migration those tables are gone; reconcile used to crash on `ALTER TABLE scene_subtitles ...` and leave the app unable to start.

### Changed

- **Identify → video folders** now loads `GET /video/series/:id` before each TMDB seek, builds a `localSeasons` payload from the user’s on-disk episodes, and uses a longer seek timeout so multi-season shows can finish TMDb season fetches. The quick-identify preview field list renames the misleading **seasonNumber** toggle to **seasonCount** (TMDb’s total season count, not a single season index).
- Folder view in `scenes-page-client.tsx` reads from the new `entityTerms.sceneFolders` / `sceneFolder` / `uncategorizedScenes` keys, producing headings like **Series**, **Movies**, and **Videos in this series** instead of the old Scene / Scene Folder copy. The section title, folder-list heading, child-series subsection, breadcrumb navigation segment for `/videos`, and the filter-bar view-mode tooltip all flow through the new terminology so future renames only need to touch `terminology.ts`.
- Scattered user-visible "scenes" strings across the web app retargeted to "Videos": performers filter chip (`Scenes: 5+` → `Videos: 5+`), StashBox phash contributions ("5 scenes linked" → "5 videos linked"), the empty-state message on the same tab, tags page "Min usage" label, search page empty-state copy, collection entity-type label dropdown, dashboard recent-activity redacted SFW label, and job-dashboard "Scene media pipeline" section title (now "Video media pipeline").
...
- New `docs/videos-to-series-rollout.md` — operator playbook for cutting the destructive finalize phase. Covers the code/environment prerequisites, the "safe to finalize" checklist, how to trigger finalize (UI + API), backup + rollback procedure, user-facing copy, sequencing dependencies, and open risks. Tracked under APP-26.

### Removed

- Deleted the orphan `apps/web/src/components/routes/scene-folder-detail-client.tsx` component — nothing has imported it since the `/scene-folders` route files were removed.
