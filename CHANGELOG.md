...
- **Collections: cross-type content groups with dynamic rules and playlist playback.** Collections group scenes, galleries, images, and audio tracks into curated sets. Three modes: manual (hand-picked items), dynamic (auto-populated by a condition builder with AND/OR/NOT logic trees), and hybrid (both). The condition builder supports filters on all entity fields — tags, performers, studios, rating, date, duration, resolution, codec, and more — with a live preview panel showing matching items before saving. Dynamic collections refresh automatically during library scans. The collection detail page offers mixed gallery, grouped-by-type, and playlist view modes. The playlist player features a unified queue with auto-advance across content types (video plays to completion, images show for a configurable duration, audio plays to completion), shuffle, loop, and a queue sidebar. An "Add to Collection" modal can be triggered from any entity page to manually assign items — manual assignments persist even when dynamic rules wouldn't match them.
