...
- Trickplay sprite generation now extracts all frames in a single ffmpeg pass using the `fps` filter instead of spawning one ffmpeg process per frame — ~50× fewer process spawns for a typical video.
