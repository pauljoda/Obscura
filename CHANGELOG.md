...
## [Unreleased]
### What's New
- Adaptive HLS now advertises Jellyfin-style bitrate quality levels and shows the active Auto/manual quality beside the player's Adaptive HLS chip, so users can see what auto selected without confusing raw-height labels like `1920p`.
...
### Changed
- Adaptive HLS master playlists now use Jellyfin's bitrate-oriented quality ladder, preserving native 4K output when appropriate while avoiding raw source-height variant names.
