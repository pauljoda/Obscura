...
- The entity grid's search/sort toolbar at the top and the pagination strip at the bottom now both stick to the edges of the page-level scrolling container. When the page is short enough that everything fits, they appear in their natural positions; once scrolling brings them to the container edge they lock in and the cards scroll between them, so the controls are always reachable while browsing a large library.
