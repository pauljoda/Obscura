...
- **Collections: database schema and typed contracts.** New `collections` and `collection_items` tables support manual, dynamic, and hybrid collections that span scenes, galleries, images, and audio tracks. Includes a full rule tree schema for dynamic condition building (AND/OR/NOT with nested groups), polymorphic collection items with manual/dynamic source tracking, slideshow settings, and cover art modes. Route paths, DTOs, and a `collection-refresh` worker queue are defined in the shared contracts package.
...
- **Synthetic library-root folders now display the actual library label even if an old custom scene-folder name exists on that row.** When the new root-folder setting is enabled, scene folder cards, breadcrumbs, and detail views now show the configured library name (for example `Main Library`) instead of a previously edited custom folder title that happened to share the same root path.
