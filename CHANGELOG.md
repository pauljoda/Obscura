...
- **Terminology sweep.** The folder view of the Videos page no longer shows "Scene folders" / "Uncategorized scenes" — it now reads "Series" and "Movies", matching the new model. Dashboard section titles, performers filter chips, StashBox contribution summaries, search placeholder copy, and collection rule-builder labels all say "Videos" instead of "Scenes". The view-mode icon buttons on the filter bar also have proper `title` tooltips so the series / grid / list toggles are legible.
