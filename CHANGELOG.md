...
- Sidebar footer "Dev Tools" link replaced with "Design Language" link pointing to `/design-language`; design-language page back-link now returns to the dashboard.
- Dev stack cleaned up for v2 — Vite dev server on port 8008 proxies `/api` and `/assets` to the .NET backend on port 8010, eliminating hardcoded port hacks in the API client. VS Code "Full Stack" launch now starts Vite with HMR alongside the .NET API and Worker for live frontend development. Removed conflicting `dev:app` script that built a static bundle and ran the API on port 8008.
