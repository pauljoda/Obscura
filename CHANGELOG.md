...
## [Unreleased]
### What's New
- Adaptive HLS now advertises quality levels up to the source/native height and shows the active Auto/manual quality beside the player's Adaptive HLS chip, so users can see what auto selected.
...
- Adaptive HLS can now try native ffmpeg encoder profiles such as Apple VideoToolbox and Linux VA-API, with software x264 kept as the reliable default and fallback.
...
### Changed
- Adaptive HLS master playlists now include 1440p, 2160p, and nonstandard native-height variants when the source supports them, with refreshed cache metadata so existing virtual playlists are rebuilt.
