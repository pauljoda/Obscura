...
### Changed
- `EntityDetail` hero now uses a two-zone layout: the sharp banner at the top fades into a vertically-flipped reflection that serves as the content zone background. The reflection image is positioned absolutely and flipped in-place so the banner's bottom edge mirrors seamlessly into the lower zone, with a frosted-glass blur overlay and bottom fade to the page background. Poster-blur mode (no banner) fits tightly to content with a smooth 64px blur across the full backdrop. Gradient mode collapses to a compact layout with no reserved image space.
