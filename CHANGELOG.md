...
- **Switching between Direct and HLS playback now reattaches subtitles to the new source.** Toggling the quality menu between Direct and adaptive HLS used to leave the subtitle overlay stuck on cues from the previous source (or empty) because the existing `<track>` elements kept stale cue buffers after the MSE source swap. The `<track>` elements now include `streamMode` / `src` / `directSrc` in their React key, so any source swap unmounts and remounts them — the browser then treats the tracks as freshly attached and refetches the VTT against the new media element state. The cuechange subscription effect also re-runs on `streamMode` changes so the new `TextTrack` instance is picked up.
- **Subtitles now render again under the new HLS pipeline, and "Off" actually turns them off.** Two regressions introduced alongside the adaptive-HLS work: (1) when the player handed the video element off to hls.js, the `<track>` cue buffers were wiped by the MSE attach but the subtitle-mode effect kept `t.mode = "hidden"` without flipping it, so the browser never re-fetched the cues and the overlay stayed empty on HLS playback. The effect now parks every subtitle track at `disabled` on every apply and then re-activates the selected one, which forces the browser to refetch the VTT after a source swap; it also re-runs on `src` / `directSrc` / `streamMode` changes and re-applies on `loadedmetadata`. (2) Clicking "Off" in the CC menu set the controlled `activeSubtitleTrackId` to `null`, but the player's library-level auto-enable effect treated `null` as "nothing picked yet" and immediately re-selected the preferred track — so the menu flipped straight back on. The player now takes a new `subtitleChoiceLocked` prop; `scene-detail.tsx` sets it whenever the user explicitly picks a track *or* the saved per-scene localStorage preference is hydrated (including the `__off__` sentinel), and the auto-enable pass hard-stops in both cases. The saved preference is also now honoured on mount — switching scenes with no saved value still runs the auto-pick once, but scenes that were explicitly toggled off stay off.
