...
- Saved filter-preset support on `/galleries` and `/images`. Presets live in localStorage under `obscura-galleries-filter-presets` and `obscura-images-filter-presets`, use the shared `createFilterPresets` helper, and show via the same `FilterPresetDropdown` component used elsewhere. The gallery / image filter panel also now exposes rating and date sections; the video-specific sections (resolution, duration, codec, playback, library flags) are suppressed for these entities.
