...
### Changed
- `EntityDetail` hero now uses a two-zone layout: the banner image is 80% sharp and visible at the top with a CSS mask that fades the bottom 20% into a vertically-flipped blurred reflection of the banner underneath. The reflection mirrors the banner's bottom edge downward for a seamless transition. Poster-blur mode (no banner) fits tightly to content with a smooth 64px blur across the full backdrop. Gradient mode collapses to a compact layout with no reserved image space.
