...
- The entity grid's search/sort toolbar at the top and the pagination strip at the bottom now both float over the page-level scroll, anchored to the visible top and bottom edges. Cards scroll cleanly behind them with a small breathing-room gap above the toolbar, and the inner-viewport dual-scroll was removed so the docked controls no longer clip the top of cards that scroll under them.
