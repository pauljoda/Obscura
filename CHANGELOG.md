...
- **Legacy installs now upgrade cleanly.** The startup migrator no longer crashes on older push-managed databases that pre-date recent tables (e.g. `scene_folders`) or whose original `drizzle-kit push` only created part of the baseline. It now heals any missing baseline tables, probes the live schema to figure out how far an install has actually progressed, and lets the normal migrator apply any remaining migrations on top — no database reset required.
...
- Legacy installs whose 0000 baseline was only partially created (e.g. `library_roots` exists but `scene_subtitles` is missing) no longer crash later migrations with `relation "scene_subtitles" does not exist`. The migrator now runs a `healBaseline` pass on every boot when core tables are present, idempotently replaying 0000's `CREATE TABLE` / `CREATE INDEX` / `ADD CONSTRAINT` statements as `IF NOT EXISTS` / duplicate-tolerant forms so any gaps in the baseline are filled before the drizzle migrator walks the journal. Heal runs outside the one-shot bridge block so installs that were already seeded by the previous bridge still get healed.
