...
- Local dev stack restarts must reproduce the canonical VS Code workflow commands directly from the shell; agents do not need to drive the VS Code UI. Do not start a second API, worker, or Vite instance on an alternate port for live verification. When the running stack needs to be refreshed, first tell the user that the dev stack is being rebooted, run `pnpm dev:kill` (equivalent VS Code task: `Obscura: Kill Orphans`) to clear orphaned .NET/Vite/Docusaurus processes and dev ports, then run the same commands as `Obscura: Full Stack`: `docker compose -f infra/docker/docker-compose.yml up -d postgres`, `pnpm --filter @obscura/web-svelte dev`, `dotnet build apps/backend/Obscura.slnx`, `dotnet run --project apps/backend/src/Obscura.Api/Obscura.Api.csproj`, and `dotnet run --project apps/backend/src/Obscura.Worker/Obscura.Worker.csproj`. Keep Vite, API, and Worker in long-running shell sessions so the user can test the refreshed stack on the normal dev ports.
...
- Ready-for-review handoff must leave the canonical full stack running. When work is complete, announce the refresh, run `pnpm dev:kill` to remove stale processes, then run the `Obscura: Full Stack` command-equivalent shell workflow above so the user can test immediately without manually killing or restarting servers.
