...
This contract adapts "Clean .NET Domain + EF Core Infrastructure Guidelines" version 1.0, prepared May 18, 2026, for Obscura. It is the backend direction for new work unless a later architecture decision explicitly replaces it.

## Baseline

Obscura backend work follows:
...
`Obscura.Domain` contains entities, aggregate roots, value objects, domain services, domain events, business exceptions, and invariants. Domain objects should expose intent-bearing methods such as `Rename`, `Rate`, `Archive`, `AddChild`, or `MarkWatched` rather than requiring callers to mutate unrelated public properties in the right order.
...
`Obscura.Application` contains use cases and ports. A meaningful state-changing operation should become a command or handler with a business name, such as `ApplyEntityMetadata`, `StartLibraryScan`, `RefreshCollection`, or `UpdatePlaybackProgress`.
...
`Obscura.Infrastructure` implements technical details: EF Core `DbContext`, entity type configuration, repository implementations, migrations, file/media tooling, provider clients, queue storage, clocks, and other external adapters.
...
`Obscura.Api` owns HTTP: endpoint composition, request DTOs, response DTOs, status mapping, authentication/authorization wiring, OpenAPI metadata, and exception mapping. Endpoints should be thin and should not return domain entities directly.
...
`Obscura.Worker` should call the same application handlers and infrastructure services as the API. One job execution should get one dependency-injection scope and one short-lived EF context/unit of work.

## EF Core Rules

- EF Core is the persistence adapter, not the domain model.
- Fluent EF configuration lives in `apps/backend/src/Obscura.Infrastructure/Persistence`.
- Migrations live in `apps/backend/src/Obscura.Infrastructure/Persistence/Migrations`.
...
- Every endpoint uses explicit request and response contracts from `Obscura.Contracts`.
...
- `@obscura/contracts` remains frontend compatibility only while surfaces migrate.
...
## Obscura Entity Direction

The current v2 entity work should be steered away from a global graph abstraction.
...
- Domain behavior lives in `Obscura.Domain` and is covered by domain tests when it can regress.
- Use-case orchestration lives in `Obscura.Application` or an intentional infrastructure adapter for technical workflows.
- EF mapping, migrations, and SQL live in `Obscura.Infrastructure`.
- API endpoints expose DTOs from `Obscura.Contracts`.
