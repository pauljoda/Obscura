...
public sealed class FilesEndpointTests : IDisposable {
    ...
    private readonly DirectoryInfo _tempRoot = Directory.CreateTempSubdirectory("obscura-files-api-");
    ...
    public async Task ChildrenEndpointRejectsTraversalOutsideRoot() {
        ...
        var problem = await response.Content.ReadFromJsonAsync<Obscura.Contracts.System.ApiProblem>();
