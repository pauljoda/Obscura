...
public sealed class FilesServiceTests : IDisposable {
    private readonly DirectoryInfo _tempRoot = Directory.CreateTempSubdirectory("obscura-files-");
    ...
    public async Task DeletePermanentlyRemovesFilesInsideRoot() {
        ...
        await service.DeleteAsync(new FileDeleteRequest(_rootId, "delete-me.txt"), CancellationToken.None);
    ...
    public async Task StorageBlocksWriteThroughSymlinkedFolders() {
        ...
        var outside = Directory.CreateTempSubdirectory("obscura-files-outside-");
        try {
