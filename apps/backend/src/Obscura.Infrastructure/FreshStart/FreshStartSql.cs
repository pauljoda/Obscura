...
public static class FreshStartSql
{
    ...
    /// <summary>
    /// Deletes entity_files rows whose role is anything other than <c>source</c>.
    /// After a fresh migration, the only entity_files that should exist are source
    /// media files — all generated assets (thumbnails, previews, trickplay, etc.)
    /// are regenerated by the first library rescan. This cleanup catches stale rows
    /// left by older import SQL that may have been compiled into a running binary.
    /// </summary>
