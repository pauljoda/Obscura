...
public static partial class LegacyMediaImportSql
{
    ...
    private static readonly string ImageTagsImport = $$"""
            IF to_regclass('public.image_tags') IS NOT NULL THEN
                INSERT INTO v2.entity_relationship_links (entity_id, relationship_code, label, target_entity_id, target_kind_code, sort_order, created_at)
                SELECT image_id, 'tags', 'Tags', tag_id, '{{EntityKindRegistry.Tag.Code}}', 0, now()
    ...
    private static readonly string ImagePerformersImport = $$"""
            IF to_regclass('public.image_performers') IS NOT NULL THEN
                ...
                SELECT image_id, 'cast', 'Cast', performer_id, '{{EntityKindRegistry.Person.Code}}', 0, jsonb_build_object('role', '{{EntityKindRegistry.Person.Code}}'), now()
