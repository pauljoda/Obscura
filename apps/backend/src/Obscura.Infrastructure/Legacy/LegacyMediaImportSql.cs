...
public static class LegacyMediaImportSql
{
    public static readonly string Import = $$"""
        ...
        BEGIN
            IF to_regclass('public.collections') IS NOT NULL THEN
                ...
                SELECT id, '{{IEntityKind.Collection.Code}}', name, created_at, updated_at
            ...
            IF to_regclass('public.galleries') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.Gallery.Code}}', title, created_at, updated_at
                ...
                WHERE studio_id IS NOT NULL
                  AND EXISTS (SELECT 1 FROM v2.entities studio WHERE studio.id = studio_id AND studio.kind_code = '{{IEntityKind.Studio.Code}}')
                ON CONFLICT (entity_id) DO UPDATE SET
                    studio_id = EXCLUDED.studio_id;

                INSERT INTO v2.entity_hierarchy_links (parent_entity_id, child_entity_id, relationship, sort_order, created_at)
                SELECT parent_id, id, '{{IEntityRelationship.NestedGallery.Code}}', 0, created_at
                FROM public.galleries
            ...
            IF to_regclass('public.images') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.Image.Code}}', title, created_at, updated_at
                FROM public.images
                ON CONFLICT (id) DO UPDATE SET
                    kind_code = EXCLUDED.kind_code,
                    title = EXCLUDED.title,
                ...
                INSERT INTO v2.entity_hierarchy_links (parent_entity_id, child_entity_id, relationship, sort_order, created_at)
                SELECT gallery_id, id, '{{IEntityRelationship.GalleryImage.Code}}', sort_order, created_at
                FROM public.images
            ...
            IF to_regclass('public.books') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.Book.Code}}', title, created_at, updated_at
                FROM public.books
                ON CONFLICT (id) DO UPDATE SET
                    kind_code = EXCLUDED.kind_code,
                    title = EXCLUDED.title,
            ...
            IF to_regclass('public.audio_libraries') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.AudioLibrary.Code}}', title, created_at, updated_at
                ...
                INSERT INTO v2.entity_hierarchy_links (parent_entity_id, child_entity_id, relationship, sort_order, created_at)
                SELECT parent_id, id, '{{IEntityRelationship.NestedAudioLibrary.Code}}', 0, created_at
                FROM public.audio_libraries
                WHERE parent_id IS NOT NULL
                ON CONFLICT (parent_entity_id, child_entity_id, relationship) DO UPDATE SET
                    sort_order = EXCLUDED.sort_order;
            ...
            IF to_regclass('public.audio_tracks') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.AudioTrack.Code}}', title, created_at, updated_at
                ...
                INSERT INTO v2.entity_hierarchy_links (parent_entity_id, child_entity_id, relationship, sort_order, created_at)
                SELECT library_id, id, '{{IEntityRelationship.AudioTrack.Code}}', sort_order, created_at
                FROM public.audio_tracks
            ...
            IF to_regclass('public.collection_items') IS NOT NULL THEN
                INSERT INTO v2.entity_hierarchy_links (parent_entity_id, child_entity_id, relationship, sort_order, created_at)
                SELECT item.collection_id, item.entity_id, '{{IEntityRelationship.CollectionItem.Code}}', item.sort_order, item.added_at
                FROM public.collection_items item
                WHERE EXISTS (SELECT 1 FROM v2.entities collection WHERE collection.id = item.collection_id AND collection.kind_code = '{{IEntityKind.Collection.Code}}')
            ...
            IF to_regclass('public.audio_track_markers') IS NOT NULL THEN
                ...
                WHERE EXISTS (SELECT 1 FROM v2.entities entity WHERE entity.id = marker.track_id AND entity.kind_code = '{{IEntityKind.AudioTrack.Code}}')
                ON CONFLICT (id) DO UPDATE SET
                    title = EXCLUDED.title,
            ...
            IF to_regclass('public.gallery_tags') IS NOT NULL THEN
                ...
                WHERE EXISTS (SELECT 1 FROM v2.entities tag WHERE tag.id = tag_id AND tag.kind_code = '{{IEntityKind.Tag.Code}}')
            ...
            IF to_regclass('public.gallery_performers') IS NOT NULL THEN
                ...
                SELECT gallery_id, performer_id, '{{IEntityKind.Person.Code}}', NULL, 0, now()
                FROM public.gallery_performers
                WHERE EXISTS (SELECT 1 FROM v2.entities person WHERE person.id = performer_id AND person.kind_code = '{{IEntityKind.Person.Code}}')
                ON CONFLICT (entity_id, person_entity_id, role) DO NOTHING;
            END IF;

            IF to_regclass('public.image_performers') IS NOT NULL THEN
                INSERT INTO v2.entity_credit_links (entity_id, person_entity_id, role, character, sort_order, created_at)
                SELECT image_id, performer_id, '{{IEntityKind.Person.Code}}', NULL, 0, now()
            ...
            IF to_regclass('public.book_performers') IS NOT NULL THEN
                INSERT INTO v2.entity_credit_links (entity_id, person_entity_id, role, character, sort_order, created_at)
                SELECT book_id, performer_id, '{{IEntityKind.Person.Code}}', NULL, 0, now()
            ...
            IF to_regclass('public.audio_library_performers') IS NOT NULL THEN
                INSERT INTO v2.entity_credit_links (entity_id, person_entity_id, role, character, sort_order, created_at)
                SELECT library_id, performer_id, '{{IEntityKind.Person.Code}}', NULL, 0, now()
            ...
            IF to_regclass('public.audio_track_performers') IS NOT NULL THEN
                INSERT INTO v2.entity_credit_links (entity_id, person_entity_id, role, character, sort_order, created_at)
                SELECT track_id, performer_id, '{{IEntityKind.Person.Code}}', NULL, 0, now()
    ...
    public static readonly string Counts = $$"""
        SELECT
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.Image.Code}}') AS images_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.Gallery.Code}}') AS galleries_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.Book.Code}}') AS books_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.AudioLibrary.Code}}') AS audio_libraries_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.AudioTrack.Code}}') AS audio_tracks_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.Collection.Code}}') AS collections_imported,
            ((SELECT COUNT(*)::int FROM v2.entity_hierarchy_links WHERE relationship IN ('{{IEntityRelationship.NestedGallery.Code}}', '{{IEntityRelationship.GalleryImage.Code}}', '{{IEntityRelationship.NestedAudioLibrary.Code}}', '{{IEntityRelationship.AudioTrack.Code}}')) +
             (SELECT COUNT(*)::int FROM v2.entity_hierarchy_links WHERE relationship = '{{IEntityRelationship.CollectionItem.Code}}') +
