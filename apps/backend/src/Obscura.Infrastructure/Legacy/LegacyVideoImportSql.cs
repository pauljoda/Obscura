...
public static class LegacyVideoImportSql
{
    public static readonly string Import = $$"""
        ...
        BEGIN
            IF to_regclass('public.tags') IS NOT NULL THEN
                ...
                SELECT id, '{{IEntityKind.Tag.Code}}', name, created_at, updated_at
                FROM public.tags
            ...
            IF to_regclass('public.studios') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.Studio.Code}}', name, created_at, updated_at
                FROM public.studios
                ON CONFLICT (id) DO UPDATE SET
                    kind_code = EXCLUDED.kind_code,
                    title = EXCLUDED.title,
            ...
            IF to_regclass('public.performers') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.Person.Code}}', name, created_at, updated_at
            ...
            IF to_regclass('public.video_series') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.VideoSeries.Code}}', COALESCE(custom_name, title), created_at, updated_at
            ...
            IF to_regclass('public.video_movies') IS NOT NULL THEN
                INSERT INTO v2.entities (id, kind_code, title, created_at, updated_at)
                SELECT id, '{{IEntityKind.Video.Code}}', title, created_at, updated_at
            ...
            IF to_regclass('public.video_episodes') IS NOT NULL THEN
                ...
                SELECT series_id, id, '{{IEntityRelationship.Episode.Code}}', (season_number * 10000) + COALESCE(episode_number, absolute_episode_number, 0), created_at
                FROM public.video_episodes
            ...
            IF to_regclass('public.video_markers') IS NOT NULL THEN
                ...
                WHERE EXISTS (SELECT 1 FROM v2.entities entity WHERE entity.id = marker.entity_id AND entity.kind_code = '{{IEntityKind.Video.Code}}')
                ON CONFLICT (id) DO UPDATE SET
                    title = EXCLUDED.title,
            ...
            IF to_regclass('public.video_subtitles') IS NOT NULL THEN
                ...
                WHERE EXISTS (SELECT 1 FROM v2.entities entity WHERE entity.id = subtitle.entity_id AND entity.kind_code = '{{IEntityKind.Video.Code}}')
            ...
            IF to_regclass('public.video_series_performers') IS NOT NULL THEN
                ...
                SELECT series_id, performer_id, '{{IEntityKind.Person.Code}}', character, COALESCE("order", 0), now()
            ...
            IF to_regclass('public.video_movie_performers') IS NOT NULL THEN
                INSERT INTO v2.entity_credit_links (entity_id, person_entity_id, role, character, sort_order, created_at)
                SELECT movie_id, performer_id, '{{IEntityKind.Person.Code}}', character, COALESCE("order", 0), now()
            ...
            IF to_regclass('public.video_episode_performers') IS NOT NULL THEN
                INSERT INTO v2.entity_credit_links (entity_id, person_entity_id, role, character, sort_order, created_at)
                SELECT episode_id, performer_id, '{{IEntityKind.Person.Code}}', character, COALESCE("order", 0), now()
    ...
    public static readonly string Counts = $$"""
        SELECT
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.VideoSeries.Code}}') AS series_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.Video.Code}}') AS videos_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.Person.Code}}') AS people_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.Tag.Code}}') AS tags_imported,
            (SELECT COUNT(*)::int FROM v2.entities WHERE kind_code = '{{IEntityKind.Studio.Code}}') AS studios_imported,
