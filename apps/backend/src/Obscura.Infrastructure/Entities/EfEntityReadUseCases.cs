...
public sealed class EfEntityReadUseCases(ObscuraDbContext db, EfEntityRepository repository)
{
    ...
    public async Task<IEntityCard?> GetDetailAsync(Guid id, string kind, CancellationToken cancellationToken)
    {
        ...
        return kind switch
        {
            ...
            "person" when entity is Person person => new PersonDetail(card.Id, card.Kind, card.Title, card.ParentEntityId, card.SortOrder, card.Capabilities, card.ChildrenByKind, card.Relationships, person.Disambiguation, person.Gender, person.Country, person.Ethnicity, person.EyeColor, person.HairColor, person.Height, person.Weight, person.Measurements, person.Tattoos, person.Piercings),
