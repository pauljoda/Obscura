...
public sealed class CapabilityPosition(IEnumerable<CapabilityPosition.Item>? items = null)
    : CollectionCapability<CapabilityPosition.Item, string>(items, StringComparer.Ordinal) {
    /// <summary>
    ...
    public sealed record Item(string Code, int Value, string? Label = null) : ICapabilityItem<string> {
        /// <inheritdoc />
    ...
    public void Set(string code, int value, string? label = null) => Set(new Item(code, value, label));
}
