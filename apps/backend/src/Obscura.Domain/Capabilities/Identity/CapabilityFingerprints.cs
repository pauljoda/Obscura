...
public sealed class CapabilityFingerprints(IEnumerable<CapabilityFingerprints.Item>? items = null)
    : CollectionCapability<CapabilityFingerprints.Item, FingerprintAlgorithm>(items) {
    /// <summary>
    ...
    public sealed record Item(FingerprintAlgorithm Algorithm, string Value) : ICapabilityItem<FingerprintAlgorithm> {
        /// <inheritdoc />
        public FingerprintAlgorithm Key => Algorithm;
    }
    ...
    /// <param name="value">Hash or fingerprint value.</param>
    public void Set(FingerprintAlgorithm algorithm, string value) => Set(new Item(algorithm, value));
}
