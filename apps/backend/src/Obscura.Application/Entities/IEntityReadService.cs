...
public interface IEntityReadService {
    ...
    /// </summary>
    Task<EntityCard?> GetAsync(Guid id, CancellationToken cancellationToken);

    /// <summary>
    /// Gets thumbnails for the requested identifiers while preserving the caller's
    /// requested order.
    /// </summary>
    Task<EntityThumbnailBatchResponse> GetThumbnailsAsync(IReadOnlyList<Guid> ids, CancellationToken cancellationToken);
    ...
    /// </summary>
    Task<IEntityCard?> GetDetailAsync(Guid id, string kind, CancellationToken cancellationToken);
}
