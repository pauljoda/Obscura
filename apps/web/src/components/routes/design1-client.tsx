...
export function Design1Client({ scenes, galleries, images }: Design1ClientProps) {
  ...
  const activeHero = featuredScenes[currentIndex];
          ...
          <div className="absolute bottom-0 left-0 w-full p-8 md:p-16 z-30 flex flex-col justify-end">
            <div className="max-w-3xl space-y-4">
              <div className="flex items-center gap-3 text-accent-500 font-mono text-sm tracking-widest uppercase">
                ...
                {activeHero?.durationFormatted && (
                  <>
                    <span className="text-text-muted">•</span>
                    <span>{activeHero.durationFormatted}</span>
                  </>
                )}
                ...
                {featuredScenes.map((scene, index) => {
                  ...
                  return (
                    <div
                      key={`info-${scene.id}`}
                      className={cn(
                        "absolute bottom-0 left-0 w-full transition-all duration-normal",
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 snap-x snap-mandatory scrollbar-hide">
              {recentScenes.map((scene, i) => (
                <div key={scene.id} className="flex-none w-72 md:w-80 snap-start">
                  <SceneCard
                    scene={sceneListItemToCardData(scene, "/design1")}
                    variant="grid"
                    index={i}
                  />
                </div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 snap-x snap-mandatory scrollbar-hide">
              {galleries.map((gallery) => (
                <div key={gallery.id} className="flex-none w-48 md:w-56 snap-start">
                  <GalleryEntityCard
                    gallery={galleryListItemToCardData(gallery, "/design1")}
                    variant="grid"
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 snap-x snap-mandatory scrollbar-hide">
              {images.map((image) => (
                <div key={image.id} className="flex-none w-64 md:w-72 snap-start">
                  <ImageEntityCard
                    image={imageItemToCardData(image, "/design1")}
                    variant="grid"
                  />
                </div>
              ))}
