...
export default function TagsPage() {
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      // scenes or recent — sort by scene count
  ...
  // Gradient classes for tag cards (matching media card pattern)
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2.5">
          {filtered.map((tag, i) => (
            <TagCard
              ...
              tag={tag}
              maxCount={maxCount}
              gradientClass={gradientClasses[i % 8]}
            />
          ))}
        </div>
      )}
  ...
  return (
    <div className={accent ? "surface-stat-accent px-3 py-2.5" : "surface-stat px-3 py-2.5"}>
      ...
      <div
        className={
          accent
            ? "text-lg font-semibold text-text-accent leading-tight truncate"
            : "text-lg font-semibold text-text-primary leading-tight truncate"
        }
      >
        {value}
      </div>
