...
async function apiJson<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const response = await fetch(v2ApiPath(path), { ...init, headers });
  if (!response.ok) {
...
export function fetchV2PluginProviders(): Promise<PluginProvider[]> {
  return apiJson("/plugins");
}

export function installV2Plugin(provider: string): Promise<PluginProvider> {
