...
export type ObscuraPluginIndexEntry = PluginIndexEntryDto & {
  localPath?: string;
...
export function fetchObscuraPluginIndex(
  options: { refresh?: boolean } = {},
): Promise<ObscuraPluginIndexEntry[]> {
  return apiJson(`/plugins/obscura-index${options.refresh ? "?refresh=1" : ""}`);
}
...
export function installObscuraPlugin(
  pluginId: string,
