<script lang="ts">
  ...
  import type { VideoRow } from "$lib/v1/identify/scrape-types-v1";
  ...
  let { row, expanded, onToggleExpand, onAccept, onDismiss, onReview, onSeekSingle }: Props = $props();
</script>
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex items-center gap-2 mt-0.5">
        <span class="text-text-disabled text-[0.65rem]">
          {row.video.durationFormatted ?? "\u2014"}
        </span>
        {#if row.video.resolution}
          <span class="pill-accent px-1 py-0 text-[0.55rem]">{row.video.resolution}</span>
        {/if}
        {#if row.normalized?.studioName}
          <span class="text-text-accent text-[0.65rem]">{row.normalized.studioName}</span>
        {/if}
        {#if row.matchedScraper && row.status !== "pending"}
          <span class="text-text-disabled text-[0.6rem] font-mono">via {row.matchedScraper}</span>
        {/if}
      </div>
    </div>
