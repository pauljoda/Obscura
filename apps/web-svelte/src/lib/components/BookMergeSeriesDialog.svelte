<script lang="ts">
  ...
  import { mergeBooksIntoSeries } from "$lib/v1/api/media-v1";
  ...
  interface Props {
    ...
    onClose: () => void;
    onMerged: (bookId: string) => void | Promise<void>;
  }

  let { open, books, onClose, onMerged }: Props = $props();
  ...
  function suggestedTitle(items: BookListItemDto[]) {
  ...
  function resetFromSelection() {
    error = null;
    const baseTitle = suggestedTitle(books);
    title = baseTitle;
    rows = books.map((book, index) => ({
      id: book.id,
      originalTitle: book.title,
      title: book.chapterCount === 1 ? `${baseTitle} #${String(index + 1).padStart(2, "0")}` : book.title,
      sequence: index + 1,
    }));
  }

  $effect(() => {
    const key = open ? books.map((book) => book.id).join("|") : "";
  ...
  async function submit() {
    ...
    try {
      ...
      const response = await mergeBooksIntoSeries({
        title: title.trim(),
        books: ordered.map((row, index) => ({
          id: row.id,
      ...
      error = err instanceof Error ? err.message : "Failed to merge books";
    } finally {
      saving = false;
    }
  }
  ...
  >
    <div class="flex h-full w-full max-w-3xl flex-col border-l border-border-subtle bg-surface-2 shadow-2xl">
      <div class="flex items-start justify-between gap-4 border-b border-border-subtle px-5 py-4">
        <div class="min-w-0">
          <h2 class="flex items-center gap-2 font-heading text-base text-text-primary">
            <FolderInput class="h-4 w-4 text-text-accent" />
            Merge into book
          </h2>
          <p class="mt-1 text-[0.76rem] text-text-muted">
            {rows.length} chapter{rows.length === 1 ? "" : "s"} from {books.length} selected book{books.length === 1 ? "" : "s"}
          </p>
        </div>
        <button
          type="button"
      ...
      <div class="flex-1 space-y-5 overflow-y-auto px-5 py-5">
        ...
        <label class="block space-y-1.5">
          <span class="text-kicker">Book folder name</span>
          <input
