      ...
      <div class="flex items-start gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="mt-1.5 text-[0.78rem] leading-relaxed text-text-muted">
            {#if showDiskOption}
              Remove the selected {noun} from Obscura, or remove the library entries and source
              file{count === 1 ? "" : "s"} from disk.
            {:else}
              Remove the selected {noun} from Obscura. Generated files and associations will also
              be cleaned up.
            {/if}
          </p>
