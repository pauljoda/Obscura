<script lang="ts">
  ...
  import type { NormalizedMovieResult } from "@obscura/contracts";
  import {
  ...
  let { result, scrapeResultId, movieId, onAccepted, onAcceptAndNext }: Props = $props();
  ...
  let mask = $state<AcceptFieldMask>(allOn(MOVIE_FIELDS));
...
<div class="flex flex-col">
  <div class="space-y-4 p-5">
      ...
      <div class="min-w-0 flex-1 space-y-2">
        ...
        {#if result.releaseDate}
          <p class="text-[0.7rem] text-text-muted">
            Released: {result.releaseDate}{#if result.runtime} · {result.runtime}min{/if}
          </p>
        {/if}
        {#if result.overview}
        ...
        {#if result.genres.length > 0}
          <div class="flex flex-wrap gap-1">
            {#each result.genres as g (g)}
              <span class="tag-chip tag-chip-default text-[0.55rem]">{g}</span>
            {/each}
          </div>
        {/if}
      </div>
    ...
    <FieldMaskGrid fields={MOVIE_FIELDS} {mask} onToggle={toggleField} />
  </div>
  <div
