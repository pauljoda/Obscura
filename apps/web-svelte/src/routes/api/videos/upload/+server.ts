...
import { uploadVideoEpisodeWrite, uploadVideoMovieWrite } from "@obscura/app-core";
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const file = files[0]!;
  const input = {
    filename: file instanceof File && file.name ? file.name : "video-upload",
    mimetype: file.type,
  ...
  try {
    ...
    return json(await uploadVideoMovieWrite(db, libraryRootId, input));
  } catch (err) {
