<script lang="ts">
  ...
  let detail = $state<V2FileDetail | null>(null);
  ...
  async function loadChildren(meta: FileTreeNodeMeta): Promise<void> {
    ...
    loadingTree = true;
    try {
      const response = await fetchV2FileChildren(meta.rootId, meta.path);
  ...
  async function loadDetail(meta: FileTreeNodeMeta): Promise<void> {
    ...
    error = null;
    try {
      detail = await fetchV2FileDetail(meta.rootId, meta.path);
    } catch (loadError) {
      detail = null;
  ...
  async function createFolder(meta: FileTreeNodeMeta): Promise<void> {
    ...
    if (!name?.trim()) return;
    await createV2FileFolder({ rootId: meta.rootId, parentPath: meta.path, name: name.trim() });
  ...
  async function renameFile(meta: FileTreeNodeMeta, proposedName?: string): Promise<void> {
    ...
    await renameV2File({ rootId: meta.rootId, path: meta.path, name: name.trim() });
  ...
  async function deleteFile(meta: FileTreeNodeMeta): Promise<void> {
    ...
    await deleteV2File(meta.rootId, meta.path);
    selectedTreePath = null;
    detail = null;
    await loadRoots();
  }
  ...
  async function uploadItems(items: V2FileUploadItem[]): Promise<void> {
    const target = directoryTarget(selectedMeta);
    if (!target || items.length === 0) return;
    try {
      await uploadV2Files(target.rootId, target.path, items);
      const nextLoaded = new Set(loadedKeys);
      nextLoaded.delete(loadedKey(target));
      loadedKeys = nextLoaded;
      await loadChildren(target);
  ...
  async function collectDroppedFiles(dataTransfer: DataTransfer | null): Promise<V2FileUploadItem[]> {
  ...
  async function traverseEntry(entry: FileSystemEntry, basePath: string): Promise<V2FileUploadItem[]> {
