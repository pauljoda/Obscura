...
import { fetchPerformers } from "$lib/v1/server/media-v1";
...
export const load: PageServerLoad = async ({ cookies, url, depends, fetch }) => {
  ...
  return {
    ...
    order,
    nsfwMode,
    surfacePrefs: await loadFormFactorUiPrefObjects("surface:performers", {}, ":prefs"),
    viewPrefs: await loadUiPrefObject("performers:view", { cols: 3 }),
  };
};
