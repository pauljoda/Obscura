<script lang="ts">
  ...
  const studio = $derived.by(() => {
    if (!library) return null;
    const cap = getCapability(library.capabilities, "studio");
    return cap?.value ?? null;
  });

  const credits = $derived.by(() => {
    if (!library) return [];
    const cap = getCapability(library.capabilities, "credits");
    return cap?.people ?? [];
  });
  ...
  const childCards = $derived.by((): EntityThumbnailCard[] => {
    if (!library) return [];
    return library.children.map((child) => {
...
<div class="detail-page">
  <a href="/v2/audio" class="back-link">
