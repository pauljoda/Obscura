<script lang="ts">
  ...
  import { Layers, Star } from "@lucide/svelte";
  import {
    fetchV2Entities,
  ...
  const totalLabel = $derived(
    items.length === 1 ? "1 series" : `${items.length.toLocaleString()} series`,
  );
  ...
  async function loadInitial() {
    ...
    try {
      const response = await fetchV2Entities({ kind: "video-series" });
...
<section class="space-y-5">
  ...
  {#if loadState === "loading"}
    <div class="series-grid" aria-busy="true">
  ...
  {:else if loadState === "error"}
    <div class="notice surface-error">
      <p>{errorMessage ?? "The v2 series library could not be loaded."}</p>
      <button type="button" onclick={() => void loadInitial()}>Retry</button>
    </div>
  {:else if items.length === 0}
    <div class="notice">
      <p>No v2 series entities are available yet.</p>
    </div>
  {:else}
