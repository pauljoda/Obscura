<script lang="ts">
  ...
  let entities = $state<V2EntityCard[]>([]);
  let selectedEntityIds = $state<string[]>([]);
  let activeEntity = $state<V2EntityCard | null>(null);
  ...
  async function install(provider: PluginProvider) {
    error = null;
    try {
      const installed = await installV2Plugin(provider.id);
  ...
  async function saveAuth(provider: PluginProvider) {
    ...
    try {
      ...
      await saveV2PluginAuth(provider.id, values);
  ...
  async function runIdentify(entity: V2EntityCard, candidate?: EntitySearchCandidate) {
...
<section class="identify-page">
  ...
  <section class="provider-strip">
    {#if availableProviders.length === 0}
      <div class="empty-row">No v2 providers for {KIND_LABELS[kind]}.</div>
    {:else}
